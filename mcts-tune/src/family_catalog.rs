...
register_family! {
    ...
    // Power-UCT: plain Ucb1 selection, uniform playout, `PowerMeanBackprop`
    // backup with a tunable exponent. The select<->backprop pairing is free
    // here (no `needs_posterior` coupling, unlike the `bayes_*` rows), so the
