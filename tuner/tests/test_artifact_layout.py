...
def test_task_identity_is_deterministic_and_a_retry_gets_a_distinct_identity():
    first = _identity()
    assert first.task_id == task_id_for(first.attempt_id, first.task_sequence, first.pair_id)
...
def test_descriptor_names_are_canonical_and_sort_in_task_sequence_order():
    names = [descriptor_filename(sequence, _identity(sequence).task_id) for sequence in (19, 2, 11)]
    assert sorted(names) == [
        descriptor_filename(sequence, _identity(sequence).task_id) for sequence in (2, 11, 19)
    ]
...
def test_paths_are_exact_relative_to_the_artifact_root_and_relocatable(tmp_path: Path):
    ...
    assert descriptor.members.trace == PurePosixPath("tasks") / identity.task_id / "trace.jsonl"
    assert layout.task_member_path(descriptor.members.complete).relative_to(layout.root) == Path(
        "tasks", identity.task_id, "complete.json"
    )
...
def test_identity_rejects_empty_traversal_and_oversize_values(attempt_id, sequence, pair_id):
...
def test_owner_declarations_assign_one_writer_and_keep_the_ingestor_read_only():
    owners = {declaration.relative_pattern: declaration.owner for declaration in ARTIFACT_OWNERSHIP}
    assert owners["attempt.json"] == "coordinator"
    assert owners["descriptors/<19-digit-task-sequence>-<task-id>.json"] == "coordinator"
...
def test_manifest_advertises_layout_schema_without_physical_task_details(
    ...
    assert semantic["schema_versions"]["artifact_layout"] == ARTIFACT_LAYOUT_SCHEMA_VERSION
