...
import type {
  ...
  Project,
  RunDetail,
} from "../packages/bench/src/types.js";
...
} from "../packages/bench/src/experiment-export.js";
...
describe("persisted experiment components", () => {
  it("keeps all repeated removal controls visible and synchronizes indexed JSON after removal", async () => {
    ...
    const edited = JSON.parse(JSON.stringify(store.state.experimentDraft!)) as NonNullable<
      BenchState["experimentDraft"]
    >;
    ...
    await vi.waitFor(() => {
      expect(store.state.experimentDraft?.spec.games[1]?.game_config).toEqual({
        marker: "second-game",
      });
      expect(store.state.experimentDraft?.spec.variants[1]?.config).toEqual({
        marker: "second-variant",
      });
    });
    ...
    await vi.waitFor(() => {
      ...
      expect(store.state.experimentDraft?.spec.games[0]?.game_config).toEqual({
        marker: "second-game",
      });
      expect(store.state.experimentDraft?.spec.variants[0]?.config).toEqual({
        marker: "second-variant",
      });
      expect((document.getElementById("game-config-0") as HTMLTextAreaElement).value).toContain(
        "second-game",
      );
      expect((document.getElementById("variant-config-0") as HTMLTextAreaElement).value).toContain(
        "second-variant",
      );
      expect(screen.getByRole("button", { name: "Remove game 1" })).toBeDisabled();
      expect(screen.getByRole("button", { name: "Remove variant 1" })).toBeDisabled();
  ...
  it("renders a project shell with labelled controls and keeps an empty name unsubmitable", async () => {
    const store = createStore<BenchState, BenchAction, BenchEnv>(
      initialBenchState(),
      benchReducer,
      noOpEnv,
    );
  ...
  it("keeps malformed strategy JSON visible, localizes the error, and re-enables saving after correction", async () => {
    ...
    await vi.waitFor(() =>
      expect(store.state.experimentDraft?.spec.baseline.config).toEqual({ depth: 2 }),
    );
  ...
  it("associates indexed server validation errors with repeated editor controls", () => {
    ...
    state.experimentDraft = {
      ...
      spec: {
        version: 1,
        games: [
          { game: "nim", game_config: null },
          { game: "druid", game_config: { size: 7 } },
        ],
        baseline: { id: "baseline", label: "Baseline", config: {} },
        variants: [
          { id: "variant-1", label: "Variant 1", config: {} },
          { id: "variant-2", label: "Variant 2", config: {} },
        ],
        budgets: [
          { kind: "iterations", value: 5 },
  ...
  it("uses tuner metadata for game defaults and preserves a positive budget when changing budget kinds", async () => {
    ...
    state.experimentDraft = {
      ...
      spec: {
        version: 1,
        games: [{ game: "nim", game_config: { stones: 7 } }],
        baseline: { id: "baseline", label: "Baseline", config: {} },
        variants: [{ id: "variant", label: "Variant", config: {} }],
        budgets: [{ kind: "iterations", value: 9 }],
        rounds_per_cell: 1,
        base_seed: 42,
        max_parallel_cells: 1,
      },
    ...
    await vi.waitFor(() =>
      expect(store.state.experimentDraft?.spec.games[0]).toEqual({
        game: "druid",
        game_config: { size: 5 },
      }),
    );
    fireEvent.change(screen.getByRole("combobox", { name: "Budget kind" }), {
      target: { value: "time_per_move_ms" },
    });
    await vi.waitFor(() =>
      expect(store.state.experimentDraft?.spec.budgets[0]).toEqual({
        kind: "time_per_move_ms",
        value: 9,
      }),
    );
  ...
  it("traverses project, experiment, launch, progress, and cell inspection with mocked effects", async () => {
    ...
    const experiment: Experiment = {
      ...
      spec: {
        ...
        baseline: { id: "baseline", label: "Baseline", config: {} },
        variants: [{ id: "variant", label: "Variant", config: {} }],
        budgets: [{ kind: "iterations", value: 1 }],
        rounds_per_cell: 1,
        base_seed: 42,
        max_parallel_cells: 1,
      },
    ...
    const env: BenchEnv = {
      listProjects: () => Effect.send([project]),
      createProject: () => Effect.send(project),
      getProject: () => Effect.send(project),
      updateProject: () => Effect.send(project),
      listExperiments: () => Effect.send([experiment]),
      createExperiment: () => Effect.send(experiment),
      getExperiment: () => Effect.send(experiment),
      updateExperiment: () => Effect.send(experiment),
      launchExperiment: () => Effect.send({ run_id: "run-1", pid: 1, log_path: "/tmp/log.jsonl" }),
      getRunCells: () => Effect.send([cell]),
      listRuns: () => Effect.send([]),
      getRun: () => Effect.send({ ...detail, status: "completed" }),
      getRunLog: () =>
        Effect.send({
          lines: Array.from({ length: 502 }, (_, index) => `log-${index}`),
          next_offset: 14,
        }),
      getRunStdout: () => Effect.send(""),
      getLeaderboard: () => Effect.send([]),
      fetchCommitTrends: () => Effect.send({}),
      launchRun: () => Effect.none(),
      stopRun: () => Effect.none(),
      getBenchKinds: () => Effect.none(),
      getTunerKinds: () => Effect.none(),
    ...
    render(() => (
      <>
        ...
        <ExperimentRunDetail store={store} />
      </>
    ));
    ...
    fireEvent.input(screen.getByLabelText("Experiment name"), {
    ...
    await vi.waitFor(() =>
      expect(dispatched).toContainEqual({
        ...
        response: { run_id: "run-1", pid: 1, log_path: "/tmp/log.jsonl" },
      }),
    );
    await vi.waitFor(() => expect(store.state.activeTab).toBe("runs"));
    await vi.waitFor(() =>
      expect(
        screen.getByRole("button", { name: /nim, 1 iterations, Variant/ }),
      ).toBeInTheDocument(),
    );
    ...
    expect(screen.getByText("Budget kind", { selector: "dt" }).parentElement).toHaveTextContent(
      "iterations",
    );
    expect(screen.getByText("Budget value", { selector: "dt" }).parentElement).toHaveTextContent(
      "1",
    );
    expect(screen.getByText("Paired rounds", { selector: "dt" }).parentElement).toHaveTextContent(
      "1",
    );
    expect(screen.getByText("Planned games", { selector: "dt" }).parentElement).toHaveTextContent(
      "2",
    );
    expect(screen.getByText("Completed games", { selector: "dt" }).parentElement).toHaveTextContent(
      "1",
    );
    ...
    expect(
      screen.getByText("Game configuration", { selector: "h3" }).parentElement,
    ).toHaveTextContent("null");
    expect(
      screen.getByText("Candidate configuration", { selector: "h3" }).parentElement,
    ).toHaveTextContent("{}");
    expect(
      screen.getByText("Baseline configuration", { selector: "h3" }).parentElement,
    ).toHaveTextContent("{}");
  ...
  it("renders a mocked 2x2x3 run, filters source games by cell, rejects stale polls, and stops", async () => {
    ...
    const spec = {
      ...
      budgets: [
        { kind: "iterations" as const, value: 11 },
        { kind: "time_per_move_ms" as const, value: 23 },
      ],
      rounds_per_cell: 1,
      base_seed: 99,
    ...
    const cells: ExperimentCell[] = Array.from({ length: 12 }, (_, ordinal) => ({
      ...
      cell_seed: 1000 + ordinal,
      game: ordinal < 6 ? "game-a" : "game-b",
      game_config: { board: ordinal < 6 ? "a" : "b" },
      variant_id: `v${(ordinal % 3) + 1}`,
      variant_label: `V${(ordinal % 3) + 1}`,
      candidate_config: { variant: ordinal },
      baseline_id: "base",
      baseline_label: "Base",
      baseline_config: { family: "ucb1" },
      budget:
        ordinal % 2 === 0
          ? { kind: "iterations", value: 11 }
          : { kind: "time_per_move_ms", value: 23 },
      rounds: 1,
      planned_games: 2,
      completed_games:
        statuses[ordinal] === "completed"
          ? 2
          : statuses[ordinal] === "running" || statuses[ordinal] === "failed"
            ? 1
            : 0,
      ...
      ended_at: null,
      error:
        statuses[ordinal] === "failed"
          ? "variant rejected"
          : statuses[ordinal] === "cancelled"
            ? "run stopped"
            : null,
      wins: 1,
      losses: 0,
    ...
    const env: BenchEnv = {
      ...noOpEnv,
      getRunGames: (runId, limit, cellId) => {
        calls.push([runId, limit, cellId]);
        return Effect.send(
          cellId
            ? [
                {
      ...
      stopRun: (runId) => {
        ...
        return Effect.send({ run_id: runId, status: "stopped" });
      },
    };
    const state = initialBenchState();
    ...
    const secondCell = container.querySelector(
      ".projects-matrix-table tbody tr:first-child td:nth-child(3) button",
    ...
    expect(container.querySelector(".projects-matrix-cell")).toHaveAttribute(
      "aria-pressed",
      "false",
    );
    ...
    store.dispatch({
      ...
      detail,
      trials: [],
      cells: cells.map((cell) => (cell.cell_id === tailedCell.cell_id ? tailedCell : cell)),
    });
    await vi.waitFor(() =>
      expect(
        screen.getByRole("button", { name: /game-a, 11 iterations, V2/ }),
      ).toHaveAccessibleName(expect.stringContaining("2/2 games")),
    );
    ...
    store.dispatch({
      ...
      lines: [],
      nextOffset: 0,
      detail: { ...stoppedDetail, run_id: "grid-run" },
      trials: [],
      cells: cells.map((cell) => ({
        ...cell,
        status: cell.status === "pending" || cell.status === "running" ? "cancelled" : cell.status,
        error: cell.status === "pending" || cell.status === "running" ? "run stopped" : cell.error,
      })),
    });
    await vi.waitFor(() =>
  ...
  it("derives the first valid matrix cell as selected without fetching it", () => {
    ...
    const makeCell = (cellId: string, game: string, variantId: string): ExperimentCell => ({
      ...
      cell_seed: 1,
      game,
      game_config: { board: game },
      variant_id: variantId,
      variant_label: variantId.toUpperCase(),
      candidate_config: { variantId },
      baseline_id: "base",
      baseline_label: "Base",
      baseline_config: {},
    ...
    state.openRun = {
      runId: detail.run_id,
      detail,
      cells: [unexpected, valid],
      games: [],
      trials: [],
      tail: { lines: [], offset: 0, active: true, error: null, idleAttempts: 0, failures: 0 },
    };
    const store = createStore<BenchState, BenchAction, BenchEnv>(state, benchReducer, {
      ...noOpEnv,
      getRunGames,
    });
  ...
  it("exports every loaded cell through the component effect, including pending and failure recovery", async () => {
    ...
    const makeCell = (cellId: string): ExperimentCell => ({
      ...
      variant_id: "v1",
      variant_label: "V1",
      candidate_config: { depth: 2 },
      baseline_id: "base",
      baseline_label: "Base",
      baseline_config: { depth: 1 },
      budget: spec.budgets[0]!,
      rounds: 1,
      planned_games: 2,
      completed_games: 1,
    ...
    await vi.waitFor(() => {
      ...
      expect(
        document.querySelector(".projects-run-export-actions [role=status]"),
      ).toHaveTextContent("Preparing download");
    });
    expect(requests[0]).toEqual([
      "experiment-run-export.json",
      "application/json",
  ...
  it("passes the selected trace across the replay boundary and clears it when the run changes", async () => {
    ...
    const Spectator: Component<BenchSpectatorProps> = (props) => {
      ...
      return (
        <div data-testid="fake-replay">
          {props.runId}:{props.cellId}:{props.initialGameSeq}
        </div>
      );
    };
    const state = initialBenchState();
    ...
    await vi.waitFor(() =>
      expect(screen.getByTestId("fake-replay")).toHaveTextContent("run-a:cell-a:7"),
    );
    expect(spectatorProps.at(-1)).toEqual({
      runId: "run-a",
      game: "game-a",
      kind: "experiment",
      live: false,
    ...
    await vi.waitFor(() =>
      expect(screen.getByTestId("fake-replay")).toHaveTextContent("run-b:cell-b:8"),
    );

    cleanup();
    const noSpectatorStore = createStore<BenchState, BenchAction, BenchEnv>(
      state,
      benchReducer,
      noOpEnv,
    );
