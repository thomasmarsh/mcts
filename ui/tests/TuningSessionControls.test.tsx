...
const control = (overrides: Partial<TuningSessionListItem["control"]> = {}) => ({
  ...
  continuation: {
    ...
    consumed_trial_count: 2,
    remaining_trial_count: 3,
...
const response = (
  ...
  overrides: Partial<TuningSessionCommandResponse> = {},
  ...
  bench_run_id: null,
  signal: null,
  budget: { previous_target_trial_count: 5, delta: 2, target_trial_count: 7 },
...
function setup(
  ...
  const env = {
    listTuningSessions: () => {
      lists += 1;
      return Effect.send({ schema_version: 1 as const, sessions });
    },
    getTuningAnalysisOverview: () => {
      analyses += 1;
      return Effect.send(overview(sessions[0]!.control));
    },
  ...
  store.dispatch({
    ...
    action: { tag: "selectSession", sessionId: sessions[0]!.session_id },
  });
...
describe("logical tuning session controls", () => {
  it("renders only projected commands and announces a pending Stop", async () => {
    ...
    const { store } = setup({
      stop: () =>
        Effect.fromPromise(
          () =>
            new Promise<TuningSessionCommandResponse>((done) => {
              resolve = done;
            }),
        ),
    });
  ...
  it("validates and previews an additive target, then submits active extension without workers", async () => {
    ...
    const { store, lists, analyses } = setup({
      budget: (_id, body) => {
        requests.push(body);
        return Effect.send(
          response(
    ...
    expect(store.getState()().tuningNavigation.list.snapshot?.sessions[0]?.target_trial_count).toBe(
      8,
    );
  ...
  it("starts an idle extension only on explicit request and leaves the current selection unchanged until Open", async () => {
    const idleControl = control({
      continuation: { ...control().continuation, active_attempt_id: null },
      allowed_commands: [
        { command: "resume", allowed: true, denial_reason: null },
        { command: "add_budget", allowed: true, denial_reason: null },
      ],
    });
    const requests: unknown[] = [];
    const { store } = setup({
      sessions: [session("session-a", idleControl)],
      budget: (_id, body) => {
        requests.push(body);
        return Effect.send(
          response(
            control({
              version: 5,
              continuation: {
                ...idleControl.continuation,
                target_trial_count: 7,
                remaining_trial_count: 5,
              },
            }),
            { status: "starting", attempt_id: "attempt-new", bench_run_id: "run-new" },
          ),
        );
      },
    });
  ...
  it("resumes only when the server projects Resume and announces the new attempt", async () => {
    const idleControl = control({
      continuation: { ...control().continuation, active_attempt_id: null },
      allowed_commands: [{ command: "resume", allowed: true, denial_reason: null }],
    });
    const requests: unknown[] = [];
    const { store } = setup({
      sessions: [session("session-a", idleControl)],
      resume: (_id, body) => {
        ...
        return Effect.send(
          response(
            control({
              version: 5,
              continuation: { ...idleControl.continuation, active_attempt_id: null },
            }),
            { status: "resuming", attempt_id: "attempt-resumed", bench_run_id: "run-resumed" },
          ),
        );
      },
    });
  ...
  it("keeps a transport command id for retry and reports a replayed success", async () => {
    ...
    const { store } = setup({
      stop: (_id, body) => {
        ...
        calls += 1;
        return calls === 1
          ? Effect.fromPromise(() => Promise.reject(new Error("offline")))
          : Effect.send(
              response(control({ version: 5 }), { command_id: body.command_id, replay: true }),
            );
      },
    });
    await screen.findByRole("button", { name: "Stop" });
    ...
    await vi.waitFor(() =>
      expect(screen.getByText("Stop succeeded (replayed request).")).toBeInTheDocument(),
    );
  ...
  it("reports a launch error while retaining the returned target and does not move selection", async () => {
    const launchedControl = control({
      version: 5,
      continuation: { ...control().continuation, target_trial_count: 7, remaining_trial_count: 5 },
    });
    const { store } = setup({
      budget: () =>
        Effect.send(
          response(launchedControl, {
            status: "launch_failed",
            launch_error: "spawn failed",
            attempt_id: "attempt-new",
            bench_run_id: "run-new",
          }),
        ),
    });
  ...
  it("drops an old session command result after a rapid session change without changing the new selection", async () => {
    let resolve!: (value: TuningSessionCommandResponse) => void;
    const secondControl = control({
      version: 9,
      continuation: { ...control().continuation, active_attempt_id: null },
    });
    const { store } = setup({
      sessions: [session("session-a"), session("session-b", secondControl)],
      stop: () =>
        Effect.fromPromise(
          () =>
            new Promise<TuningSessionCommandResponse>((done) => {
    ...
    store.dispatch({
      tag: "tuningNavigation",
      action: { tag: "selectSession", sessionId: "session-b" },
    });
    resolve(response(control({ version: 5 })));
    await vi.waitFor(() =>
      expect(store.getState()().tuningNavigation.commands["session-a"]?.status).toBe("succeeded"),
    );
    expect(store.getState()().tuningNavigation.selection).toEqual({
      sessionId: "session-b",
      attemptId: null,
      trialId: null,
      pairId: null,
