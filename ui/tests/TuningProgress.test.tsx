...
import { benchReducer, initialBenchState, type BenchAction, type BenchEnv, type BenchState, type TuningAnalysisOverview, type TuningTrialDetail } from "@mcts/bench";
...
const counts = { total: 7, queued: 1, running: 1, terminal: 5, completed: 2, failed: 1, pruned: 1, cancelled: 1 };
const point = (trial_id: string, trial_number: number, trial_status: string, bracket_id: string | null, resource: number, score: number, mu: number, sigma: number, reason = "max_pairs") => ({
  trial_id, trial_number, trial_status, bracket_id, resource, score, rating: { mu, sigma }, outcome: trial_status === "pruned" ? "prune" : trial_status === "complete" ? "complete" : "continue", reason, pruning_exempt: false, rung_resource: bracket_id === null ? null : resource,
});
...
const overview: TuningAnalysisOverview = {
  ...
  coverage: { trials: counts, reports: 9, pairs: { total: 12, running: 1, complete: 9, failed: 2, unmatched_pool_revisions: 0 }, points: { total: 9, returned: 7, sampled: true } },
  bracket_resources: [
...
const selectedDetail: TuningTrialDetail = {
  ...
  cursor: { session_sequence: 2 },
  trial: {
    trial_id: "outside-sample", trial_number: 8, attempt_id: "attempt", state: "complete", config: {}, score: 7, rating: { mu: 20, sigma: 2 }, reason: "max_pairs", failure: null, pairs: [],
    reports: [
      { completed_pairs: 2, rating: { mu: 16, sigma: 3 }, score: 2, score_formula_version: 1, conservative_k: 3, decision: { outcome: "continue", reason: "startup_exempt", pruning_exempt: true, bracket_id: "gamma", rung_resource: 2 }, reported_at: "" },
      { completed_pairs: 4, rating: { mu: 20, sigma: 2 }, score: 7, score_formula_version: 1, conservative_k: 3, decision: { outcome: "complete", reason: "max_pairs", pruning_exempt: false, bracket_id: "gamma", rung_resource: 4 }, reported_at: "" },
    ],
  },
};
...
describe("tuning Progress view", () => {
  it("renders ordered bracket facets, terminal markers, rung guides, tables, and sampled disclosure", () => {
    ...
    expect(screen.getByText(/Exact full-population counts: 7 trials and 9 reports/)).toBeInTheDocument();
    expect(screen.getAllByRole("img")).toHaveLength(4);
    expect(screen.getAllByRole("table")).toHaveLength(4);
    expect(screen.getAllByTestId("progress-mark")).toHaveLength(9);
    expect(screen.getAllByRole("heading", { level: 5 }).map((heading) => heading.textContent)).toEqual(["Unassigned", "alpha", "beta", "gamma"]);
  ...
  it("selects a stable trial with mouse and keyboard without changing the active tab", () => {
    ...
    const mark = screen.getAllByTestId("progress-mark").find((element) => element.getAttribute("data-trial-id") === "complete")!;
  ...
  it("filters facets and keeps metric and scale controls stable through a progress refresh", async () => {
    ...
    expect(store.getState()().tuningNavigation).toMatchObject({ progressMetric: "sigma", progressScale: "local", filters: { bracket: "beta", state: "pruned", family: "rave" } });
  ...
  it("explains legacy, empty, and error progress states without falling back to the old chart", () => {
    const legacy = { ...overview, coverage: { ...overview.coverage, reports: 0, points: { total: 0, returned: 0, sampled: false } }, points: [] };
