...
const session = {
  session_id: "session-trials",
  game: "nim",
  label: "Trials",
  status: "completed",
  target_trial_count: 6,
  counts,
...
function row(number: number, state = "complete", has_detail = true): TuningTrialSummary {
  return {
    ...
    state,
    reason: state === "complete" ? "max_pairs" : state,
    rating: state === "complete" ? { mu: 25, sigma: 1 } : null,
    score: state === "complete" ? 22 : null,
    family: number === 6 ? null : "ucb1",
    config_summary: null,
    bracket_id: number === 6 ? null : "b1",
    resource: number === 6 ? null : 4,
    pair_count: 2,
    wins: 2,
    losses: 1,
    draws: 1,
    elapsed_ms: 30,
    search_iterations_total: 40,
    search_move_time_ms: 5,
    has_detail,
  };
}
  ...
  reports: [
    {
      ...
      reported_at: "2026-08-23T00:01:00Z",
    },
  ],
  pairs: [
    {
      ...
      opponent: {
        anchor_id: "anchor-1",
        label: "Strong",
        config: { family: "rave", c: 2 },
        mu: 24,
        sigma: 1,
        provenance: "pool",
      },
      pool_snapshot_fingerprint: "pool-fp",
      pool_revision: {
        pool_snapshot_fingerprint: "pool-fp",
        display_ordinal: 2,
      ...
      games: [
        {
          ...
          plies: 20,
          elapsed_ms: 11,
          candidate: { iterations_total: 20, iterations_first_half: 10, move_time_ms: 3 },
          baseline: { iterations_total: 20, iterations_first_half: 10, move_time_ms: 3 },
          replay: {
            run_id: "run-1",
            game_seq: 11,
            has_renderer_trace: true,
...
function setup(
  getPage: (query: TuningTrialPageQuery | undefined) => TuningTrialPage,
  getDetail: () => TuningTrialDetail = () => ({
    schema_version: 1,
    trial: candidate,
    cursor: { session_sequence: 1 },
  }),
): { queries: (TuningTrialPageQuery | undefined)[]; detailCalls: string[] } {
  ...
  const env = {
    ...
    getTuningTrialPage: (_id: string, query: TuningTrialPageQuery | undefined) => {
      queries.push(query);
      return Effect.send(getPage(query));
    },
    getTuningTrialDetail: (_id: string, trialId: string) => {
      detailCalls.push(trialId);
      return Effect.send(getDetail());
    },
...
describe("paged tuning Trials view", () => {
  it("queries 50-row pages, sends filters and sorting to the server, and preserves no initial row selection", async () => {
    const { queries } = setup((query) =>
      page(query?.cursor ? [row(2)] : [row(1), row(2)], query?.cursor ? null : "next", 83),
    );
    ...
    await vi.waitFor(() =>
      expect(queries.at(-1)).toMatchObject({ q: "ucb", sort: "trial", direction: "desc" }),
    );
    fireEvent.change(screen.getByLabelText("Sort trials"), { target: { value: "score" } });
    await vi.waitFor(() =>
      expect(queries.at(-1)).toMatchObject({ q: "ucb", sort: "score", direction: "desc" }),
    );
  ...
  it("loads exactly one detail when expanded, retains selection across refreshes, and renders immutable pair evidence", async () => {
    ...
    expect(
      screen.getByText(/Not recorded — this game has no replay reference/),
    ).toBeInTheDocument();
