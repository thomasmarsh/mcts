...
const point = (
  ...
  outcome,
  reason,
  pruning_exempt: reason === "startup_exempt",
  bracket_id,
  rung_resource,
});
const overview: TuningAnalysisOverview = {
  schema_version: 1,
  policy: {
    resource: { min_pairs: 2, max_pairs: 8 },
    rating: { model: "tm", score: "mu_minus_k_sigma", sigma_stop: null, conservative_k: 3 },
    sampler: { kind: "tpe", seed: 4, deterministic: true, startup_trials: 2 },
    pruning: { enabled: true, kind: "hyperband", reduction_factor: 3, startup_trials: 2 },
  },
  ...
  coverage: {
    trials: counts,
    reports: 14,
    pairs: { total: 20, running: 2, complete: 16, failed: 2, unmatched_pool_revisions: 0 },
    points: { total: 14, returned: 4, sampled: true },
  },
  bracket_resources: [
...
describe("tuning Pruning view", () => {
  ...
  it("filters the exact Trials read from a decision segment and selects trajectories with a keyboard", () => {
    ...
    expect(store.getState()().tuningNavigation).toMatchObject({
      tab: "trials",
      filters: { reason: "hyperband_prune", state: null },
    });
    store.dispatch({ tag: "tuningNavigation", action: { tag: "setAnalysisTab", tab: "pruning" } });
    const trajectory = screen
      .getAllByTestId("pruning-trajectory")
      .find((element) => element.getAttribute("data-trial-id") === "trial-1")!;
  ...
  it("keeps selection and filters through refresh, and explains disabled, empty, and error evidence", () => {
    ...
    store.dispatch({
      ...
      action: { tag: "setTrialFilters", filters: { bracket: "bracket-10", reason: "max_pairs" } },
    });
    store.dispatch({
      tag: "tuningNavigation",
    ...
    expect(store.getState()().tuningNavigation).toMatchObject({
      tab: "pruning",
      selection: { trialId: "outside-sample" },
      filters: { bracket: "bracket-10", reason: "max_pairs" },
    });
    first.unmount();
    const disabled = {
      ...overview,
      policy: { ...overview.policy!, pruning: { ...overview.policy!.pruning, enabled: false } },
    };
    const second = render(() => <TuningPruningView store={setup(disabled)} />);
    expect(
      screen.getByText(
        "Pruning was disabled by the recorded policy; continued reports are shown without claiming a pruning decision.",
      ),
    ).toBeInTheDocument();
    second.unmount();
    const empty = {
