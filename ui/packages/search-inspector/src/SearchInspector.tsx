...
export type SearchMetric =
  "iterations" | "elapsed" | "nodes" | "meanDepth" | "maxDepth" | "ttHitRatio";
...
const warningDetail: Record<SearchWarning, string> = {
  ...
  root_parallel_pv_single_tree:
    "The principal variation represents one root-parallel tree, not the aggregate search.",
};
...
const reportReason: Record<NonNullable<SearchReport<unknown>["reason"]>, string> = {
  ...
  root_parallel_pv_single_tree:
    "Only a single root-parallel tree could provide a principal variation.",
};
...
function reportMetric<M>(
  ...
  if (key === "nodes")
    return report.warnings.includes("structural_diagnostics_omitted") ? null : report.tree_nodes;
...
const metrics: MetricDefinition<unknown>[] = [
  {
    key: "iterations",
    label: "Iterations",
    value: (r) => reportMetric(r, "iterations"),
    format: number,
  },
  {
    key: "elapsed",
    label: "Elapsed",
    value: (r) => reportMetric(r, "elapsed"),
    format: (v) => `${number(v)} s`,
  },
  { key: "nodes", label: "Nodes", value: (r) => reportMetric(r, "nodes"), format: number },
  {
    key: "meanDepth",
    label: "Mean depth",
    value: (r) => reportMetric(r, "meanDepth"),
    format: number,
  },
  {
    key: "maxDepth",
    label: "Max depth",
    value: (r) => reportMetric(r, "maxDepth"),
    format: number,
  },
  {
    key: "ttHitRatio",
    label: "TT hit ratio",
    value: (r) => reportMetric(r, "ttHitRatio"),
    format: percent,
  },
];

function TrendChart<M>(props: {
  points: SearchInspectorPoint<M>[];
  ...
  const x = (index: number) =>
    left +
    (props.points.length === 1 ? plotWidth / 2 : (index / (props.points.length - 1)) * plotWidth);
  const y = (value: number) => top + ((upper - value) / span) * plotHeight;
  const path = values.reduce(
    (d, value, index) =>
      value === null
        ? ""
        : `${d}${index === 0 || values[index - 1] === null ? "M" : "L"}${x(index)},${y(value)} `,
    "",
  );
      ...
      <For each={values}>
        {(value, index) => (
          <Show when={value !== null}>
            <circle cx={x(index())} cy={y(value!)} r="3" fill="currentColor" />
          </Show>
        )}
      </For>
      <Show
        when={hasValues}
        fallback={
          <text x={left + 6} y={top + plotHeight / 2} font-size="11">
            No values available
          </text>
        }
      >
        <>
          <text x={left - 4} y={top + 4} text-anchor="end" font-size="10">
            {props.metric.format(upper)}
          </text>
          <text x={left - 4} y={top + plotHeight} text-anchor="end" font-size="10">
            {props.metric.format(lower)}
          </text>
        </>
      </Show>
    </svg>
...
export const SearchInspector = <S, M>(props: SearchInspectorProps<S, M>) => {
  ...
  const metric = createMemo(
    () => metrics.find((entry) => entry.key === metricKey()) as MetricDefinition<M>,
  );
  const trendRows = createMemo(() => {
    const currentMetric = metric();
    return trendPoints().map((point) => ({
      point,
      value: currentMetric.value(point.report),
  ...
  return (
    <section aria-label="Final search inspector">
      <Show
        when={props.report}
        fallback={<p role="status">No final-search report is available from this legacy result.</p>}
      >
        {(report) => (
          <>
            <Show when={report().status === "unavailable"}>
              <p role="status">
                Final-search evidence unavailable.{" "}
                {reasonDetail(report().reason, "No further reason was supplied.")}
              </p>
            </Show>
            <Show when={report().status === "partial"}>
              <p role="status">
                Final-search evidence is partial.{" "}
                {reasonDetail(report().reason, "Review the warnings below.")}
              </p>
            </Show>
              ...
              <section aria-labelledby={warningHeadingId}>
                ...
                <ul>
                  <For each={report().warnings}>
                    {(warning) => <li>{warningDetail[warning]}</li>}
                  </For>
                </ul>
              </section>
            </Show>
            ...
            <section aria-labelledby={summaryHeadingId}>
              ...
              <dl>
                <dt>Iteration limit</dt>
                <dd>{nullableNumber(report().iteration_limit)}</dd>
                <dt>Time limit</dt>
                <dd>{seconds(report().time_limit_seconds)}</dd>
                <dt>Completed iterations</dt>
                <dd>{number(report().completed_iterations)}</dd>
                <dt>Termination</dt>
                <dd>{report().termination ?? "Unavailable"}</dd>
                <dt>Elapsed</dt>
                <dd>{seconds(report().elapsed_seconds)}</dd>
                <dt>Throughput</dt>
                <dd>{throughput(report().iterations_per_second)}</dd>
                <dt>Root visits</dt>
                <dd>{number(report().root_visits)}</dd>
                <dt>Tree nodes</dt>
                <dd>{number(report().tree_nodes)}</dd>
                <dt>Mean depth</dt>
                <dd>{nullableNumber(report().mean_depth)}</dd>
                <dt>Max depth</dt>
                <dd>{nullableNumber(report().max_depth)}</dd>
                <dt>Graph</dt>
                <dd>{report().graph_mode ?? "Unavailable"}</dd>
                <dt>TT reads</dt>
                <dd>{number(report().tt_reads)}</dd>
                <dt>TT writes</dt>
                <dd>{number(report().tt_writes)}</dd>
                <dt>TT hits</dt>
                <dd>{number(report().tt_hits)}</dd>
                <dt>TT hit ratio</dt>
                <dd>{hitRatio(report().tt_hit_ratio)}</dd>
              </dl>
            </section>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th scope="col">Outcome proven</th>
                  ...
                  <For each={report().actions}>
                    {(action) => (
                      <tr>
                        ...
                        <td>{number(action.visits)}</td>
                        <td>{percent(action.share)}</td>
                        <td>{number(action.mean_value)}</td>
                        <td>{action.is_proven ? "Yes" : "No"}</td>
                        <td>
                          {report().selected_action !== null &&
                          moveEquals(action.action, report().selected_action)
                            ? "Selected"
                            : ""}
                        </td>
                      </tr>
      ...
      <Show when={trendPoints().length >= 2}>
        <section aria-labelledby={trendHeadingId}>
          ...
          <label>
            ...
            <select
              value={metricKey()}
              onChange={(event) => setMetricKey(event.currentTarget.value as SearchMetric)}
            >
              <For each={metrics}>
            ...
            <tbody>
              <For each={trendRows()}>
                {(row) => (
                  <tr>
                    <td>{row.point.ply}</td>
                    <td>{row.point.player}</td>
                    <td>{rawMoveLabel(row.point.move)}</td>
                    <td>{row.value === null ? "Unavailable" : row.format(row.value)}</td>
                  </tr>
                )}
              </For>
            </tbody>
