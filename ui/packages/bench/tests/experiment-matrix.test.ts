...
const spec: ExperimentSpecV1 = {
  ...
  budgets: [{ kind: "iterations", value: 10 }, { kind: "time_per_move_ms", value: 10 }],
  rounds_per_cell: 1,
  base_seed: 42,
  max_parallel_cells: 1,
};

function cell(cellId: string, game: string, budget: ExperimentCell["budget"], variantId: string): ExperimentCell {
  return {
    cell_id: cellId, cell_seed: 1, game, game_config: {}, variant_id: variantId, variant_label: variantId,
    candidate_config: {}, baseline_id: "base", baseline_label: "Base", baseline_config: {}, budget, rounds: 1,
    planned_games: 2, completed_games: 1, status: "running", started_at: null, ended_at: null, error: null,
    wins: 1, losses: 0, draws: 0, win_rate: 1, ci_lower: 0, ci_upper: 1,
  };
}

describe("buildExperimentMatrix", () => {
  it("builds budget sections with game rows and variant columns in spec order", () => {
    const cells = spec.budgets.flatMap((budget, budgetIndex) => spec.games.flatMap((game, gameIndex) => spec.variants.map((variant, variantIndex) => cell(`cell-${budgetIndex}-${gameIndex}-${variantIndex}`, game.game, budget, variant.id))));
    const model = buildExperimentMatrix(spec, [...cells].reverse());
    expect(model.sections).toHaveLength(2);
    expect(model.sections.map((section) => [section.budget.kind, section.budget.value])).toEqual([["iterations", 10], ["time_per_move_ms", 10]]);
    expect(model.sections.every((section) => section.rows.length === 2 && section.rows.every((row) => row.cells.length === 3))).toBe(true);
  ...
  it("leaves missing coordinates visible and reports unexpected and duplicate cells deterministically", () => {
    ...
    expect(model.warnings.map((warning) => [warning.kind, warning.cellId])).toEqual([["unexpected", "unexpected"], ["duplicate", "z-duplicate"]]);
  });

  it("keeps a one-cell snapshot as a one-by-one matrix", () => {
    const oneSpec = { ...spec, games: [spec.games[0]!], budgets: [spec.budgets[0]!], variants: [spec.variants[0]!] };
    const model = buildExperimentMatrix(oneSpec, [cell("only", "game-a", oneSpec.budgets[0]!, "v1")]);
