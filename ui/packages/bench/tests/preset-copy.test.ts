...
const anchor: TuningPoolAnchor = {
  anchor_ordinal: 2,
  anchor_id: "pool/A",
  config: { family: "rave", threshold: 50, mcgs: true },
  rating: { mu: 22, sigma: 1 },
...
describe("preset copy", () => {
  it("serializes candidate and opponent snapshots in the frozen deterministic shape", () => {
    ...
    const opponentBuild = buildPresetSpec(
      opponentPresetSource(anchor, revision, { max_time_ms: 250 }),
    );
    expect(candidateBuild).toMatchObject({
      enabled: true,
      preset: {
        id: "candidate-trial_x20__x2f__x20__x41_",
        description: "Candidate snapshot from trial 7 (trial / A).",
        params: candidate.config,
        max_iterations: 10_000,
        threads: 1,
        use_transpositions: true,
      },
    });
    expect(opponentBuild).toMatchObject({
      enabled: true,
      preset: {
    ...
    if (!candidateBuild.enabled || !opponentBuild.enabled)
  ...
  it("uses the recorded mcgs parameter as capability, not its selected boolean value", () => {
    for (const [params, expected] of [
      ...
      const result = buildPresetSpec({
        kind: "candidate",
        sourceId: "source",
        sourceDescription: "Recorded.",
        params,
      });
  ...
  it("selects exactly one valid budget and reports legacy or invalid snapshots with typed reasons", () => {
    const cases: Array<{ input: PresetSource; expected: string; enabled: boolean }> = [
      ...
      },
      {
        ...
        expected: "multiple_budgets",
        enabled: false,
      },
      {
        input: { kind: "candidate", sourceId: "a", sourceDescription: "", params: null },
        expected: "legacy_missing_config",
        enabled: false,
      },
      {
        input: { kind: "candidate", sourceId: "a", sourceDescription: "", params: {} },
        expected: "missing_family",
        enabled: false,
      },
      {
        input: {
          ...
          sourceId: "a",
          sourceDescription: "",
          params: { family: "ucb1", mcgs: "yes" },
        },
        expected: "invalid_mcgs",
        enabled: false,
      },
    ];
  ...
  it("returns accessible clipboard success, rejection, and disabled states", async () => {
    ...
    await expect(copyPreset(valid, writer)).resolves.toEqual({
      status: "success",
      announcement: "Preset copied to clipboard.",
    });
    expect(writer.writeText).toHaveBeenCalledWith(
      expect.stringContaining('"id": "candidate-trial'),
    );
    await expect(
      copyPreset(valid, { writeText: vi.fn().mockRejectedValue(new Error("denied")) }),
    ).resolves.toEqual({ status: "failure", announcement: "Could not copy preset to clipboard." });
    await expect(
      copyPreset(
        buildPresetSpec({
          kind: "candidate",
