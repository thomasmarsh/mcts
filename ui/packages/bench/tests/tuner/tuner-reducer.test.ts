...
describe("tunerReducer", () => {
  it("init loads every resource and stops polling when nothing is live", () => {
    const ts = store(mockTunerEnv({ listRuns: () => Effect.send([runView({ status: "exited" })]) }));
  ...
  it("keeps polling the journal while a run is live, then stops and refreshes", () => {
    ...
    const ts = store(
      mockTunerEnv({
        listRuns: () => {
          call += 1;
          return Effect.send(call < 2 ? [runView({ status: "live" })] : [runView({ status: "exited" })]);
        },
      }),
    );
  ...
  it("optimistically inserts and opens a launched run, then tails its log", () => {
    ...
    ts.receive(
      { tag: "logLoaded", generation: 1, lines: ["cohort 0 starting"], errLines: [], nextOffset: 17 },
      (s) => {
        s.log = { lines: ["cohort 0 starting"], errLines: [], offset: 17, error: null, active: true };
      },
    );
    ts.receive({ tag: "runsLoaded", runs: [runView({ run_id: "fresh", status: "exited" })] }, (s) => {
