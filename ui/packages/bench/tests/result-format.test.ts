...
describe("result formatting", () => {
  it("presents returned rates and intervals without recalculating them", () => {
    ...
    expect(formatObservedResult({ completed_games: 4, win_rate: 0.75, ci_lower: 0.4, ci_upper: 0.9 })).toBe("75.0% (95% CI 40.0% – 90.0%)");
  });
  ...
  it("does not present the neutral default as an observation", () => {
    ...
    expect(formatLeaderboardResult({ strategy: "empty", total: 0, wins: 0, losses: 0, draws: 0, win_rate: 0.5, ci_lower: 0, ci_upper: 1 })).toBe("No games yet");
  });
