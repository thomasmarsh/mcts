...
const detail: RunDetail = {
  ...
  experiment_spec: {
    version: 1,
    games: [{ game: "nim", game_config: { label: "a,b" } }],
    baseline: { id: "base", label: "Base", config: { quote: "yes" } },
...
function cell(cellId: string): ExperimentCell {
  return {
    ...
    cell_seed: null,
    game: "nim",
    game_config: { text: "a,b" },
    variant_id: "v1",
    variant_label: 'Variant, "quoted"',
    candidate_config: { lines: "one\ntwo" },
    baseline_id: "base",
    baseline_label: "Base",
    baseline_config: { quote: "yes" },
    budget: { kind: "iterations", value: 5 },
    rounds: 1,
    planned_games: 2,
    completed_games: 0,
    status: "pending",
