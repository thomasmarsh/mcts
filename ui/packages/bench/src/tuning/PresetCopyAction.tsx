...
export const PresetCopyAction: Component<{ label: string; build: PresetBuildResult }> = (props) => {
  ...
  const announcement = () =>
    status()?.announcement ?? (props.build.enabled ? "" : props.build.reason.message);
  ...
  return (
    <span class="tuning-copy-action">
      <button
        ...
        title={unavailable() ? announcement() : `Copy ${props.label} as a preset`}
        onClick={() => void copy()}
      >
        {unavailable() ? `${props.label} unavailable` : `Copy ${props.label}`}
      </button>
      <Show when={announcement()}>
        <span class="tuning-copy-status" role="status">
          {announcement()}
        </span>
      </Show>
    </span>
