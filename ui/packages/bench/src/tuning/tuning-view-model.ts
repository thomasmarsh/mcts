...
export function selectedPair(
  ...
  return (
    selectedTrial(detail, selection)?.pairs.find((pair) => pair.pair_id === selection.pairId) ??
    null
  );
}
...
export function selectedGame(
  ...
  return (
    selectedPair(detail, selection)?.games.find((game) => game.game_id === selection.gameId) ?? null
  );
}
...
export function replayTarget(
  ...
  if (!attempt?.bench_run_id)
    return "Replay unavailable: the attempt has no associated physical run.";
  ...
  return {
    runId: attempt.bench_run_id,
    game: session.game,
    gameSeq: game.trace_game_seq,
