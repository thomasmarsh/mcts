...
function trialSummary(trial: TuningTrial): string {
  const values = [
    trial.status,
    configurationSummary(trial.config),
    `score ${formatScore(trial.score)}`,
  ];
  if (trial.mu !== null && trial.sigma !== null)
    values.push(`rating ${formatRating(trial.mu, trial.sigma)}`);
...
function pairSummary(pair: TuningPair): string {
  const values = [
    pairEvidence(pair),
    `before ${formatRating(pair.rating_before.mu, pair.rating_before.sigma)}`,
  ];
  if (pair.rating_after)
    values.push(`after ${formatRating(pair.rating_after.mu, pair.rating_after.sigma)}`);
...
function gameSummary(game: TuningGame): string {
  const trace =
    game.trace_game_seq === null ? "trace unavailable" : `trace #${game.trace_game_seq}`;
...
const GameNode: Component<NodeProps & { game: Accessor<TuningGame> }> = (props) => {
  const selected = () =>
    props.store.getState()().tuningNavigation.selection.gameId === props.game().game_id;
  return (
    <li class="tuning-tree-node" role="treeitem" aria-selected={selected()}>
      <div class="tuning-node-controls">
        <button
          class="tuning-node-select"
          onClick={() => send(props.store, { tag: "selectGame", gameId: props.game().game_id })}
        >
...
const PairNode: Component<NodeProps & { pair: Accessor<TuningPair> }> = (props) => {
  ...
  const selected = () =>
    props.store.getState()().tuningNavigation.selection.pairId === props.pair().pair_id;
  return (
    <li
      class="tuning-tree-node"
      role="treeitem"
      aria-selected={selected()}
      aria-expanded={props.expanded(id())}
    >
      <div class="tuning-node-controls">
        <Toggle
          store={props.store}
          id={id()}
          expanded={props.expanded(id())}
          label={`pair ${props.pair().pair_index + 1}`}
        />
        <button
          class="tuning-node-select"
          onClick={() => send(props.store, { tag: "selectPair", pairId: props.pair().pair_id })}
        >
...
const TrialNode: Component<NodeProps & { trial: Accessor<TuningTrial> }> = (props) => {
  ...
  const selected = () =>
    props.store.getState()().tuningNavigation.selection.trialId === props.trial().trial_id;
  return (
    <li
      class="tuning-tree-node"
      role="treeitem"
    ...
    >
      <div class="tuning-node-controls">
        <Toggle
          ...
          id={id()}
          expanded={props.expanded(id())}
          label={`trial ${props.trial().trial_number}`}
        />
        <button
          class="tuning-node-select"
          onClick={() => send(props.store, { tag: "selectTrial", trialId: props.trial().trial_id })}
        >
          Trial #{props.trial().trial_number}
          <span class="tuning-node-status">
            {trialSummary(props.trial())} · {props.trial().pairs.length} pairs
          </span>
        </button>
      </div>
      <Show when={props.expanded(id())}>
...
const AttemptNode: Component<NodeProps & { attempt: Accessor<TuningAttempt> }> = (props) => {
  ...
  const selected = () =>
    props.store.getState()().tuningNavigation.selection.attemptId === props.attempt().attempt_id;
  return (
    <li
      class="tuning-tree-node"
      role="treeitem"
    ...
    >
      <div class="tuning-node-controls">
        <Toggle
          ...
          id={id()}
          expanded={props.expanded(id())}
          label={`attempt ${props.attempt().attempt_id}`}
        />
        <button
          class="tuning-node-select"
          onClick={() =>
            send(props.store, { tag: "selectAttempt", attemptId: props.attempt().attempt_id })
          }
        >
          Attempt {props.attempt().attempt_id.slice(0, 12)}
          <span class="tuning-node-status">
            {props.attempt().status} · {trials().length} trials
          </span>
        </button>
      </div>
      <Show when={props.expanded(id())}>
...
export const TuningHierarchy: Component<TreeProps> = (props) => {
  const expanded = (id: string): boolean =>
    props.store.getState()().tuningNavigation.expandedIds.includes(id);
  return (
    <section class="tuning-panel" aria-labelledby="tuning-hierarchy-heading">
      ...
      <ul class="tuning-tree" role="tree" aria-label="Tuning evidence hierarchy">
        <Index each={props.detail.attempts}>
          {(attempt) => <AttemptNode {...props} expanded={expanded} attempt={attempt} />}
        </Index>
      </ul>
    </section>
  );
