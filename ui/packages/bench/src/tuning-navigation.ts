...
export type TuningNavigationAction =
  ...
  | {
      tag: "sessionCommandSucceeded";
      sessionId: string;
      commandId: string;
      response: TuningSessionCommandResponse;
    }
  | {
      tag: "sessionCommandFailed";
      sessionId: string;
      commandId: string;
      error: string;
      retriable: boolean;
...
function defaultFilters(): TuningTrialFilters {
  return { state: null, bracket: null, reason: null, family: null, q: null };
}
function pageState(): TuningTrialPageState {
  return {
    ...loadState<TuningTrialPage>(),
    sessionId: null,
    queryKey: null,
    cursor: null,
    previousCursors: [],
...
export function initialTuningNavigationState(): TuningNavigationState {
  return {
    ...
    overview: { ...loadState<TuningAnalysisOverview>(), sessionId: null },
    trialPage: pageState(),
    trialDetails: {},
    tab: "progress",
    ...
    trialPageLimit: DEFAULT_TRIAL_PAGE_LIMIT,
    ladderRevision: null,
    ladderAnchorKey: null,
...
function sessionCanAnalyze(state: TuningNavigationState, sessionId: string): boolean {
  return (
    state.list.snapshot?.sessions.find((session) => session.session_id === sessionId)?.capabilities
      .has_lifecycle ?? false
  );
}
function sessionIsActive(state: TuningNavigationState, sessionId: string): boolean {
  const status = state.list.snapshot?.sessions.find(
...
function pageQuery(state: TuningNavigationState): TuningTrialPageQuery {
  ...
  return {
    ...
    limit: state.trialPageLimit,
    cursor: trialPage.cursor,
  };
}
function clearDetail(state: TuningNavigationState): void {
  state.detail = {
    ...loadState<TuningSessionDetail>(),
    generation: state.detail.generation + 1,
    sessionId: null,
  };
}
function clearAnalysis(state: TuningNavigationState): void {
  state.overview = {
    ...loadState<TuningAnalysisOverview>(),
    generation: state.overview.generation + 1,
    sessionId: null,
  };
...
function requestDetail(
  ...
  return env
    .getTuningSession(sessionId)
    .map((detail): TuningNavigationAction => ({
      tag: "detailLoaded",
      generation,
      sessionId,
      detail,
...
function requestOverview(
  ...
  return env
    .getTuningAnalysisOverview(sessionId)
    .map((overview): TuningNavigationAction => ({
      tag: "overviewLoaded",
      generation,
      sessionId,
      overview,
...
function requestTrialPage(
  ...
  const snapshot =
    state.trialPage.sessionId === sessionId && state.trialPage.queryKey === queryKey
      ? state.trialPage.snapshot
      : null;
  state.trialPage = {
    ...state.trialPage,
    status: "loading",
  ...
  return env
    .getTuningTrialPage(sessionId, query)
    .map((page): TuningNavigationAction => ({
      tag: "trialPageLoaded",
      generation,
      sessionId,
      queryKey,
...
function requestTrialDetail(
  ...
  if (
    ...
    (existing.status === "loading" || (existing.status === "done" && existing.snapshot !== null))
  )
    return Effect.none();
  const generation = (existing?.generation ?? 0) + 1;
  const snapshot = existing?.sessionId === sessionId ? existing.snapshot : null;
  state.trialDetails[trialId] = {
    status: "loading",
    snapshot,
    error: null,
    generation,
  ...
  return env
    .getTuningTrialDetail(sessionId, trialId)
    .map((detail): TuningNavigationAction => ({
      tag: "trialDetailLoaded",
      generation,
      sessionId,
...
function merge(
  ...effects: (Effect<TuningNavigationAction> | null)[]
): Effect<TuningNavigationAction> | null {
  const present = effects.filter(
    (effect): effect is Effect<TuningNavigationAction> => effect !== null,
  );
...
function selectTrial(state: TuningNavigationState, trialId: string): void {
  ...
  state.selection = {
    ...state.selection,
    attemptId:
      legacyTrial?.attempt_id ??
      pageTrial?.attempt_id ??
      loadedTrial?.attempt_id ??
      state.selection.attemptId,
    trialId,
    pairId: null,
    gameId: null,
  };
...
function selectPair(state: TuningNavigationState, pairId: string): void {
  const legacyTrial = state.detail.snapshot?.trials.find((row) =>
    row.pairs.some((pair) => pair.pair_id === pairId),
  );
  const loadedTrial = Object.values(state.trialDetails)
    .map((value) => value.snapshot?.trial)
    .find((trial) => trial?.pairs.some((pair) => pair.pair_id === pairId));
  state.selection = {
    ...state.selection,
    attemptId: legacyTrial?.attempt_id ?? loadedTrial?.attempt_id ?? state.selection.attemptId,
    trialId: legacyTrial?.trial_id ?? loadedTrial?.trial_id ?? state.selection.trialId,
    pairId,
    gameId: null,
  };
  state.unavailable = null;
...
function selectGame(state: TuningNavigationState, gameId: string): void {
  const legacyTrial = state.detail.snapshot?.trials.find((row) =>
    row.pairs.some((pair) => pair.games.some((game) => game.game_id === gameId)),
  );
  const loadedTrial = Object.values(state.trialDetails)
    .map((value) => value.snapshot?.trial)
    .find((trial) =>
      trial?.pairs.some((pair) => pair.games.some((game) => game.game_id === gameId)),
    );
  ...
  state.selection = {
    ...
    attemptId: trial?.attempt_id ?? state.selection.attemptId,
    trialId: trial?.trial_id ?? state.selection.trialId,
    pairId: pair?.pair_id ?? state.selection.pairId,
    gameId,
  };
  state.unavailable = null;
}
function unavailable(
  state: TuningNavigationState,
  entity: "attempt" | "trial" | "pair" | "game",
): void {
  const selected = state.selection;
  if (entity === "attempt") state.selection = selectionForSession(selected.sessionId!);
  if (entity === "trial")
    state.selection = { ...selected, trialId: null, pairId: null, gameId: null };
...
function reconcileSelection(state: TuningNavigationState, detail: TuningSessionDetail): void {
  ...
  const trial = detail.trials.find(
    (row) => row.trial_id === selected.trialId && row.attempt_id === selected.attemptId,
  );
  ...
  if (selected.gameId && !pair?.games.some((game) => game.game_id === selected.gameId))
...
function overviewRefresh(
  ...
  return state.selection.sessionId === sessionId && sessionIsActive(state, sessionId)
    ? Effect.delay(TUNING_DETAIL_REFRESH_MS, {
        tag: "overviewRefreshTick",
        sessionId,
        generation: state.overview.generation,
      })
    : null;
}
function resetTrialPage(state: TuningNavigationState): void {
  state.trialPage = {
    ...pageState(),
    generation: state.trialPage.generation + 1,
    sessionId: state.selection.sessionId,
  };
}

function commandEffect(
  command: TuningSessionCommandState,
  sessionId: string,
  env: BenchEnv,
): Effect<TuningNavigationAction> {
  ...
  return effect
    .map((response): TuningNavigationAction => ({
      tag: "sessionCommandSucceeded",
      sessionId,
...
function applyControl(
  ...
  if (state.overview.sessionId === sessionId && state.overview.snapshot)
    state.overview.snapshot.control = control;
}
...
function refreshAfterCommand(
  ...
  return merge(
    ...
    state.selection.sessionId === sessionId && state.detail.sessionId === sessionId
      ? requestDetail(state, sessionId, env)
      : null,
  );
}
...
export function tuningNavigationReducer(
  ...
  if (action.tag === "sessionCommandSubmit") {
    ...
    if (
      (action.kind === "add_budget" &&
        (!Number.isSafeInteger(action.delta) ||
          action.delta! <= 0 ||
          typeof action.start !== "boolean" ||
          (!action.start && action.nWorkers !== undefined))) ||
      !action.commandId
    )
      return null;
  ...
  if (action.tag === "sessionCommandSucceeded") {
    const command = state.commands[action.sessionId];
    if (!command || command.commandId !== action.commandId || command.status !== "pending")
      return null;
  ...
  if (action.tag === "sessionCommandFailed") {
    ...
    command.error = action.error;
    command.retriable = action.retriable;
    return null;
  }
  ...
  if (action.tag === "listLoaded" || action.tag === "listFailed") {
    ...
    state.list =
      action.tag === "listLoaded"
        ? { ...state.list, status: "done", snapshot: action.response, error: null }
        : { ...state.list, status: "error", error: action.error };
    return null;
  }
  if (action.tag === "detailRequest")
    return state.selection.sessionId === action.sessionId
      ? requestDetail(state, action.sessionId, env)
      : null;
  if (action.tag === "detailLoaded" || action.tag === "detailFailed") {
    if (
      action.generation !== state.detail.generation ||
  ...
  if (action.tag === "overviewRequest")
    return state.selection.sessionId === action.sessionId
      ? requestOverview(state, action.sessionId, env)
      : null;
  if (action.tag === "overviewRefreshTick") {
    return action.generation === state.overview.generation &&
      action.sessionId === state.selection.sessionId &&
      sessionIsActive(state, action.sessionId)
      ? requestOverview(state, action.sessionId, env)
      : null;
  }
  if (action.tag === "overviewLoaded" || action.tag === "overviewFailed") {
    if (
      action.generation !== state.overview.generation ||
      action.sessionId !== state.selection.sessionId
    )
      return null;
    if (action.tag === "overviewFailed") {
      state.overview = { ...state.overview, status: "error", error: action.error };
      return null;
    }
    ...
    const advanced =
      priorCursor !== undefined && action.overview.cursor.session_sequence > priorCursor;
    return merge(
      state.tab === "trials" && (state.trialPage.snapshot === null || advanced)
        ? requestTrialPage(state, action.sessionId, env)
        : null,
      advanced && state.selection.trialId
        ? requestTrialDetail(state, action.sessionId, state.selection.trialId, env)
        : null,
      overviewRefresh(state, action.sessionId),
    );
  }
  if (action.tag === "trialPageRequest")
    return state.selection.sessionId === action.sessionId
      ? requestTrialPage(state, action.sessionId, env)
      : null;
  if (action.tag === "trialPageLoaded" || action.tag === "trialPageFailed") {
    if (
      action.generation !== state.trialPage.generation ||
      action.sessionId !== state.selection.sessionId ||
      action.queryKey !== state.trialPage.queryKey
    )
      return null;
    state.trialPage =
      action.tag === "trialPageLoaded"
        ? { ...state.trialPage, status: "done", snapshot: action.page, error: null }
        : { ...state.trialPage, status: "error", error: action.error };
    return null;
  }
  if (action.tag === "trialDetailRequest")
    return state.selection.sessionId === action.sessionId
      ? requestTrialDetail(state, action.sessionId, action.trialId, env)
      : null;
  if (action.tag === "trialDetailLoaded" || action.tag === "trialDetailFailed") {
    ...
    if (
      ...
      return null;
    if (action.tag === "trialDetailLoaded") {
      state.trialDetails[action.trialId] = {
        ...current,
        status: "done",
        snapshot: action.detail,
        error: null,
      };
      reconcileTrialDetail(state, action.detail);
    } else
      state.trialDetails[action.trialId] = { ...current, status: "error", error: action.error };
    return null;
  }
  if (action.tag === "selectSession") {
    ...
    state.ladderRevision = null;
    state.ladderAnchorKey = null;
    state.unavailable = null;
    clearAnalysis(state);
    return merge(
      sessionCanAnalyze(state, action.sessionId)
        ? null
        : requestDetail(state, action.sessionId, env),
      requestOverview(state, action.sessionId, env),
    );
  ...
  if (action.tag === "setAnalysisTab") {
    ...
    if (action.tab === "trials" && state.selection.sessionId && state.trialPage.snapshot === null)
      return requestTrialPage(state, state.selection.sessionId, env);
    if (
      action.tab === "game" &&
      state.selection.sessionId &&
      sessionCanAnalyze(state, state.selection.sessionId)
    ) {
      return merge(
        state.trialPage.snapshot === null
          ? requestTrialPage(state, state.selection.sessionId, env)
          : null,
        state.selection.trialId
          ? requestTrialDetail(state, state.selection.sessionId, state.selection.trialId, env)
          : null,
      );
    }
    if (action.tab === "ladder" && state.selection.sessionId && state.selection.trialId)
      return requestTrialDetail(state, state.selection.sessionId, state.selection.trialId, env);
    return null;
  }
  if (action.tag === "setProgressMetric") {
    state.progressMetric = action.metric;
    return null;
  }
  if (action.tag === "setProgressScale") {
    state.progressScale = action.scale;
    return null;
  }
  if (action.tag === "setLadderRevision") {
    state.ladderRevision = action.revision;
    return null;
  }
  if (action.tag === "selectLadderAnchor") {
    state.ladderAnchorKey = action.anchorKey;
    return null;
  }
  if (
    action.tag === "setTrialFilters" ||
    action.tag === "setTrialSort" ||
    action.tag === "setTrialPageLimit"
  ) {
    if (action.tag === "setTrialFilters") state.filters = { ...state.filters, ...action.filters };
      ...
      state.trialPageLimit = Math.max(
        1,
        Math.min(MAX_TRIAL_PAGE_LIMIT, Math.floor(action.limit) || DEFAULT_TRIAL_PAGE_LIMIT),
      );
    resetTrialPage(state);
    return state.selection.sessionId && state.tab === "trials"
      ? requestTrialPage(state, state.selection.sessionId, env)
      : null;
  }
  if (action.tag === "nextTrialPage") {
    ...
    state.trialPage.previousCursors = [...state.trialPage.previousCursors, state.trialPage.cursor];
    state.trialPage.cursor = next;
    return requestTrialPage(state, state.selection.sessionId, env);
  }
  if (action.tag === "previousTrialPage") {
    if (!state.selection.sessionId || state.trialPage.previousCursors.length === 0) return null;
    state.trialPage.cursor = state.trialPage.previousCursors.at(-1)!;
    state.trialPage.previousCursors = state.trialPage.previousCursors.slice(0, -1);
    return requestTrialPage(state, state.selection.sessionId, env);
  }
  ...
  if (action.tag === "selectTrial") {
    ...
    return (state.tab === "ladder" || state.tab === "game") &&
      state.selection.sessionId &&
      sessionCanAnalyze(state, state.selection.sessionId)
      ? requestTrialDetail(state, state.selection.sessionId, action.trialId, env)
      : null;
  }
  if (action.tag === "selectPair") selectPair(state, action.pairId);
  if (action.tag === "selectGame") selectGame(state, action.gameId);
  if (action.tag === "toggleExpanded")
    state.expandedIds = state.expandedIds.includes(action.id)
