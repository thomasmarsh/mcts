  ...
  return <section id="spectator-panel">
    ...
    <div id="spectator-layout">
      <div id="spectator-games">
        <Show when={!games.loading} fallback={<div class="log-empty">Loading games…</div>}><Show when={(games() ?? []).length > 0} fallback={<div class="log-empty">No traced games yet.</div>}><For each={games() ?? []}>{(game: GameTraceSummary) => <button class="spectator-game" classList={{ active: selectedSeq() === game.game_seq }} onClick={() => void selectGame(game.game_seq)}>#{game.game_seq} · {game.strategy_a && game.strategy_b ? `${game.strategy_a} vs ${game.strategy_b}` : props.kind === "tuner" ? `${game.ply_count} plies · tuning worker` : `${game.ply_count} plies`}</button>}</For></Show></Show>
      </div>
