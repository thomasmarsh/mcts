...
function mergeMoves(rows: GameMove[], row: GameMove): GameMove[] {
  return [...rows.filter((existing) => existing.ply !== row.ply), row].sort(
    (a, b) => a.ply - b.ply,
  );
}
...
export const SpectatorPanel: Component<SpectatorPanelProps> = (props) => {
  ...
  const selectionKey = createMemo(() =>
    JSON.stringify([props.runId, props.cellId ?? null, props.initialGameSeq ?? null]),
  );
  const [module] = createResource(
    () => props.game,
    async (game): Promise<GameKindModule<unknown, unknown, unknown> | null> => {
  ...
  const searchPoints = createMemo((): SearchInspectorPoint<unknown>[] =>
    moves().flatMap((row) =>
      row.mv === null
        ? []
  ...
  const selectedGame = createMemo(
    () => games().find((game) => game.game_seq === selectedSeq()) ?? null,
  );
  const isRendererTrace = createMemo(
    () => currentMove() !== null && typeof currentMove()!.state !== "string",
  );
  const newerGames = createMemo(() =>
    selectedSeq() === null ? 0 : games().filter((game) => game.game_seq > selectedSeq()!).length,
  );
  const newerPlies = createMemo(() =>
    currentMove() === null ? 0 : moves().filter((row) => row.ply > currentPly()).length,
  );
  ...
  async function selectGame(gameSeq: number): Promise<void> {
    ...
    try {
      ...
      if (generation !== moveGeneration || key !== selectionKey() || selectedSeq() !== gameSeq)
        return;
  ...
  createEffect(() => {
    ...
    setLiveError(null);
    const source = traceEnv.eventSource(
      `/api/bench/runs/${encodeURIComponent(runId)}/live?game_seq=${encodeURIComponent(gameSeq)}`,
    );
    source.onmessage = (event) => {
      if (
        generation !== selectedStreamGeneration ||
        key !== selectionKey() ||
        selectedSeq() !== gameSeq
      )
        return;
      try {
        ...
        if (generation === selectedStreamGeneration)
          setLiveError(`Invalid live trace event: ${String(error)}`);
      }
    };
    source.onerror = () => {
      if (generation === selectedStreamGeneration && key === selectionKey())
        setLiveError("Live trace connection lost.");
    };
  ...
  return (
    <section id="spectator-panel">
      <div id="spectator-header">
        <strong>Game traces</strong>
        <Show when={newerGames() > 0}>
          <span>{countLabel(newerGames(), "game")}</span>
        </Show>
      </div>
      <Show when={liveError()}>
        <div class="log-error">{liveError()}</div>
      </Show>
      <div id="spectator-layout">
        <div id="spectator-games">
          <Show when={gamesError()}>
            <div class="log-error">{gamesError()}</div>
          </Show>
          <Show when={!gamesLoading()} fallback={<div class="log-empty">Loading games…</div>}>
            <Show
              when={games().length > 0}
              fallback={<div class="log-empty">No traced games yet.</div>}
            >
              <For each={games()}>
                {(game) => (
                  <button
                    class="spectator-game"
                    classList={{ active: selectedSeq() === game.game_seq }}
                    onClick={() => void selectGame(game.game_seq)}
                  >
                    #{game.game_seq} ·{" "}
                    {game.strategy_a && game.strategy_b
                      ? `${game.strategy_a} vs ${game.strategy_b}`
                      : props.kind === "tuner"
                        ? `${game.ply_count} plies · tuning worker`
        ...
        <div id="spectator-board">
          <Show when={moveError()}>
            <div class="log-error">{moveError()}</div>
          </Show>
          <Show
            when={currentMove()}
            fallback={
              <div class="log-empty">
                Choose a game to inspect. Selecting it never switches to another running game.
              </div>
            }
          >
            {(row) => (
              <>
                <Show
                  when={isRendererTrace()}
                  fallback={
                    <div class="spectator-text-trace">
                      <div>
                        <strong>tuner trace</strong>
                        {selectedGame() ? ` · game #${selectedGame()!.game_seq}` : ""}
                      </div>
                      <div>Player: {row().player ?? "initial state"}</div>
                      <pre>{String(row().state)}</pre>
                      <Show when={row().mv !== null}>
                        <pre>Move: {JSON.stringify(row().mv)}</pre>
                      </Show>
                    </div>
                  }
                >
                ...
                <section class="spectator-search">
                  <SearchInspector
                    report={row().search as SearchReport<unknown> | null | undefined}
                    points={searchPoints()}
                    before={previousState()}
                    formatMove={module()?.formatMove}
                  />
                </section>
              </>
            )}
          ...
          <Show when={moves().length > 0}>
            <div id="spectator-controls">
              <button disabled={currentIndex() <= 0} onClick={() => setPly(0)}>
                First
              </button>
              <button disabled={currentIndex() <= 0} onClick={() => setPly(currentIndex() - 1)}>
                Previous
              </button>
              <span>
                Ply {currentMove()?.ply ?? 0} / {moves()[moves().length - 1]?.ply ?? 0}
                <Show when={newerPlies() > 0}> · {countLabel(newerPlies(), "ply")}</Show>
              </span>
              <button
                disabled={currentIndex() < 0 || currentIndex() >= moves().length - 1}
                onClick={() => setPly(currentIndex() + 1)}
              >
                Next
              </button>
              <button
