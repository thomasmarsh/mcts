...
/// `games/congo/presets.json` (or the file named by `CONGO_PRESETS_PATH`),
...
fn presets() -> &'static PresetTable {
    ...
    PRESETS.get_or_init(|| {
        let presets_path = env::var("CONGO_PRESETS_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/presets.json"))
            });
        PresetTable::load_from_path(&presets_path).expect("games/congo/presets.json must parse")
    })
}
