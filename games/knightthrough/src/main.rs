...
impl GameAdapter for KtAdapter {
    ...
    fn ai_move(&self, state: &Value, preset: &str) -> Result<AiMoveResult, HostError> {
        ...
        let mut ai = presets().build::<Knightthrough<8, 8>>(preset, PRESET_SEED)?;
    ...
    fn tune_eval(
        ...
        let outcome = if let Some(cfg) = baseline_config {
            ...
            mcts_tune::build_search::<Knightthrough<8, 8>>(&cfg, baseline_seed, false, &budget)?;
            mcts_tune::strategy_tune_eval(
                ...
                budget,
                move || {
                    mcts_tune::build_search::<Knightthrough<8, 8>>(
                        &cfg,
                        baseline_seed,
                        false,
                        &budget,
            ...
            mcts_tune::strategy_tune_eval(
                ...
                move || {
                    presets()
                        .build::<Knightthrough<8, 8>>("strong", PRESET_SEED)
                        .expect("games/knightthrough/presets.json's \"strong\" preset must build")
                },
                Default::default(),
                trace_path.as_deref(),
                on_game,
