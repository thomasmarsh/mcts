...
fn value_to_state(v: &Value) -> Result<Count, HostError> {
    let val = v["value"].as_i64().ok_or_else(|| HostError::bad_request("no value"))?;
    Ok(Count(val as i32))
}

fn build_easy() -> Box<dyn Search<G = CountingGame>> {
    Box::new(TreeSearch::<CountingGame, strategy::Ucb1>::new().config(
        SearchConfig::new().name("count/easy").expand_threshold(1).max_iterations(100)))
}
fn build_strong() -> Box<dyn Search<G = CountingGame>> {
    Box::new(TreeSearch::<CountingGame, strategy::Ucb1>::new().config(
        SearchConfig::new().name("count/strong").expand_threshold(0).max_iterations(5000).q_init(QInit::Loss)))
}
...
struct PresetEntry { id: &'static str, build: fn() -> Box<dyn Search<G = CountingGame>> }

struct CountAdapter;

impl GameAdapter for CountAdapter {
    fn kind(&self) -> &'static str { "count" }
    fn label(&self) -> &'static str { "Count" }
    fn description(&self) -> &'static str { "A counting game where you add or subtract toward 10." }
    ...
    fn apply(&self, state: &Value, mv: &Value) -> Result<Value, HostError> {
        let s = value_to_state(state)?;
        let m: CountMove = serde_json::from_value(mv.clone()).map_err(|e| HostError::bad_request(e.to_string()))?;
        if CountingGame::is_terminal(&s) { return Err(HostError::bad_request("game is over")); }
...
fn main() { run_stdin_stdout(CountAdapter); }