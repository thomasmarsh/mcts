...
//! not full positional superko). There is no swap rule yet. The game ends
//! when the player to move has no legal placement, and the winner is
//! whoever has more pieces on the board (a draw if equal).
...
/// positional superko: see the module docs).
