...
fn placement_failure_diagnostics(
    state: &State,
    index: usize,
    action_n: u8,
    adjacency: &TouchingAdjacency,
    ...
    let total_cells = state.total_cells();
    let mover_black = state.turn == Player::Black;
    let ground = ground_mask(state.n(), total_cells);
    let (base_black, base_white) = visible_boards(&state.occupied, &state.black);
    let cached_decision = groups_candidate_is_legal(state, index, adjacency, ground, mover_black);
    let candidate_decision = candidate_is_legal(
        state,
        index,
        adjacency,
        base_black,
    ...
    format!(
        "action generated by generate_actions must be legal\n\
         placement: Place({index}, {action_n}), coord=({col},{row},L{level}), mover={:?}\n\
         physical: occupied={}, can_place={}\n\
         legality: groups_candidate_is_legal={cached_decision:?}, candidate_is_legal={candidate_decision}, authoritative=false\n\
         state: n={}, turn={:?}, can_swap={}, occupied={:?}, black={:?}, zombie={:?}, previous={:?}\n\
         touching cached groups:\n{}\
         board:\n{state}",
        state.turn,
        ...
        state.turn,
        state.can_swap,
        state.occupied_indices(),
        state.black_indices(),
        state.zombie_indices(),
        state.previous_indices(),
        state.groups.candidate_diagnostics(index, adjacency),
    )
}
...
impl Game for Margo {
    ...
    fn apply(mut state: State, action: &Action) -> State {
        ...
        match *action {
            Action::Place(index, action_n) => {
                let adjacency = pyramid::get_adjacency(state.occupied.n());
                let index = index as usize;
                let mover_black = state.turn == Player::Black;
                let prev_occupied_count = state.occupied.count_ones();
                let (new_occupied, new_black, new_zombie) =
                    state.resolve_place(index, adjacency).unwrap_or_else(|| {
                        panic!(
                            "{}",
                            placement_failure_diagnostics(&state, index, action_n, adjacency)
                        )
                    });
