// Snapshot of `druid.rs` as it stood at commit 762b492, the parent of the
// move-splitting commit (09eca53) that replaced this file's flat
// `Move(Piece, u8)` action with a linearized `Piece`/`Orientation`/`Cell`
// phase sequence. Kept as a separate module (not merged with `druid.rs`)
// solely so `examples/strength_move_splitting.rs` can pit the pre- and
// post-linearization engines against each other inside a single binary --
// the two `Druid`/`HashedState` types are otherwise unrelated to the ones in
// `druid.rs` and should not be used for anything else. Not wired into the
// server or any preset; do not add features here.
//
// One exception, already applied: the Zobrist hash below covers
// player-to-move and both hands' remaining piece counts, not just board
// cells, matching `druid.rs`'s hash. Without that, `use_transpositions(true)`
// is unsound here -- a lintel placement can raise a cell's height by more
// than 1, which decouples "how many turns were played" from "what the board
// looks like", so two different, both-legally-reachable (board, player,
// hands) combinations can hash identically and silently alias unrelated
// MCTS transposition-table nodes. This is a correctness bug that predates
// the move-splitting split, not a feature -- mirror further fixes of that
// kind here too if any turn up, but nothing else.
//
...
// from move history) -- so the hash above omitted them. That assumption is
// false: a lintel placement raises all 3 touched cells to
...
// like". Two different, both-legally-reachable boards can then differ in
// `player` and/or hand counts while hashing identically, silently aliasing
// unrelated MCTS transposition-table nodes. Hashing `player` and both
// hands' remaining counts closes that gap.
const PLAYER_HASHES_LEN: usize = 1;
static PLAYER_HASHES: LazyZobristTable<PLAYER_HASHES_LEN> = LazyZobristTable::new(0xD403D);

/// XOR contribution of player-to-move. `Black` contributes 0, so only one
/// table entry is needed to toggle between the two players.
...
/// XOR contribution of one player's remaining count of `kind` pieces (`0` =
/// sarsens, `1` = lintels, per `piece_kind_index`) -- same bit-subset trick
/// as `cell_zobrist`, applied to a bounded counter (a hand count) instead of
/// a board cell's height. Reuses the cell-height bit width (`bits`): a
/// hand's sarsen count is bounded by exactly that value (`Hand::new`'s
/// `n * 2` is `max_cell_height`'s definition) and its lintel count (`n`)
/// never needs more bits than that, so one shared width covers both tracks.
fn hand_zobrist(player: Player, kind: usize, count: u8, bits: usize) -> u64 {
    let c = count as usize;
    if c == 0 {
        return 0;
    }
    let track = player.to_index() * 2 + kind;
    let base = track * bits;
    (0..bits).fold(0, |hash, b| {
        if c & (1 << b) != 0 {
...
/// Full from-scratch hash: board cells, player-to-move, and both players'
/// remaining hand counts -- every component the incremental update in
/// `Game::apply` maintains. Used by `HashedState::new`/`from_state` (states
/// that don't arrive via `Game::apply`, so have nothing to update
/// incrementally) and by the property test that checks the incremental
/// update stays in sync with it.
fn full_hash(state: &State, bits: usize) -> u64 {
    recompute_hash(state, bits)
        ^ player_zobrist(state.player)
        ^ hand_zobrist(Player::Black, 0, state.hand_black.sarsens, bits)
        ^ hand_zobrist(Player::Black, 1, state.hand_black.lintels, bits)
        ^ hand_zobrist(Player::White, 0, state.hand_white.sarsens, bits)
        ^ hand_zobrist(Player::White, 1, state.hand_white.lintels, bits)
}
...
impl Game for Druid {
    ...
    fn apply(mut state: Self::S, m: &Self::A) -> Self::S {
        ...
        let kind = piece_kind_index(m.0);
        let old_hand_count = match (mover, kind) {
            (Player::Black, 0) => state.0.hand_black.sarsens,
            (Player::Black, _) => state.0.hand_black.lintels,
            (Player::White, 0) => state.0.hand_white.sarsens,
            (Player::White, _) => state.0.hand_white.lintels,
        };
        ...
        hash ^= hand_zobrist(mover, kind, old_hand_count - 1, bits);
        state.1 = hash;
