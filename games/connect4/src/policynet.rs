...
#[derive(Clone, Debug)]
pub struct NTuplePolicyNet { weights: Vec<f32> }

impl Default for NTuplePolicyNet {
    fn default() -> Self { Self { weights: vec![0.0; POLICY_WEIGHTS] } }
}

impl NTuplePolicyNet {
    pub fn from_weights(weights: Vec<f32>) -> Self {
        assert_eq!(weights.len(), POLICY_WEIGHTS, "policy sidecar needs {POLICY_WEIGHTS} weights");
        Self { weights }
    }
    pub fn load(path: impl AsRef<Path>) -> std::io::Result<Self> {
        let bytes = std::fs::read(path)?;
        if bytes.len() != POLICY_WEIGHTS * 4 { return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, format!("expected {} bytes ({POLICY_WEIGHTS} f32), got {}", POLICY_WEIGHTS * 4, bytes.len()))); }
        Ok(Self::from_weights(bytes.chunks_exact(4).map(|x| f32::from_le_bytes(x.try_into().unwrap())).collect()))
    }
    ...
    fn raw_logits(&self, state: &State<6, 7>, mirrored: bool) -> [f64; 7] {
        ...
        for feat in NTupleValueNet::active_indices(state, mirrored) {
            ...
            for col in 0..7 { out[col] += row[col] as f64; }
        }
        out
    }
...
mod tests {
    ...
    fn zero_sidecar_is_uniform_over_legal_actions() {
        ...
        assert_eq!(NTuplePolicyNet::default().logits(&state, &actions), vec![0.0; 7]);
    }

    #[test]
    fn symmetry_average_is_mirror_equivariant() {
        let weights = (0..POLICY_WEIGHTS).map(|i| (i as f32 * 0.001).sin()).collect();
        ...
        for col in 0..7 { assert!((a[col] - b[6-col]).abs() < 1e-10); }
    }
    ...
    fn logits_match_python_reference_fixture() {
        let weights = (0..POLICY_WEIGHTS)
            ...
            .collect();
        let net = NTuplePolicyNet::from_weights(weights);
        let mut black = crate::BitBoard::<6, 7>::EMPTY; black.set_index(0); black.set_index(2);
        let mut white = crate::BitBoard::<6, 7>::EMPTY; white.set_index(1); white.set_index(7);
        let got = net.all_logits(&State::from_parts(black, white, crate::Player::Black, false));
        let expected = [-0.75586009, -0.75586015, -0.75585949, -0.75585961, -0.75585949, -0.75586015, -0.75586045];
        for (actual, expected) in got.into_iter().zip(expected) { assert!((actual - expected).abs() < 1e-6); }
    }
}
