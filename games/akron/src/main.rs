...
use game_akron::{Action, Akron, Player, State, DEFAULT_N, MAX_N, MIN_N};
use mcts::game::Game;
...
/// `games/akron/presets.json` (or the file named by `AKRON_PRESETS_PATH`),
...
/// doc comment). Presets are size-invariant, mirroring `games/margo`.
fn presets() -> &'static PresetTable {
    static PRESETS: OnceLock<PresetTable> = OnceLock::new();
    PRESETS.get_or_init(|| {
        let presets_path = env::var("AKRON_PRESETS_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"), "/presets.json"))
            });
        PresetTable::load_from_path(&presets_path).expect("games/akron/presets.json must parse")
    })
}
...
/// Wire encoding of a `State`, built entirely from `game_akron::State`'s
...
/// One occupied cell's display data for [`GameView`].
#[derive(Serialize)]
struct CellView {
    piece: &'static str,
}
...
fn state_to_value(s: &State) -> Value {
    serde_json::to_value(WireState {
        ...
        white_pile: s.pile(Player::White),
        black_pile: s.pile(Player::Black),
    ...
    let cells = (0..total)
        .map(|i| {
            if !s.is_occupied(i) {
                ...
                Some(CellView {
                    piece: player_name(if s.is_black(i) {
                        Player::Black
                    } else {
                        Player::White
                    }),
        ...
        .collect();
    let terminal = Akron::is_terminal(s);
    GameView {
        n: s.n() as u8,
        cells,
        white_pile: s.pile(Player::White),
        ...
        can_swap: s.can_swap(),
        winner: terminal
            .then(|| Akron::winner(s).map(|p| player_name(p).to_string()))
            .flatten(),
        terminal,
    }
}
...
impl GameAdapter for AkronAdapter {
    ...
    fn description(&self) -> &'static str {
        "A pyramidal connection game: add or relocate marbles to span your two sides of the board, with a crossing connection cutting whichever one runs underneath it."
    }
    ...
    fn legal_moves(&self, state: &Value) -> Result<Vec<Value>, HostError> {
        ...
        if !Akron::is_terminal(&s) {
            Akron::generate_actions(&s, &mut actions);
        }
        Ok(actions
            .into_iter()
    ...
    fn apply(&self, state: &Value, mv: &Value) -> Result<Value, HostError> {
        ...
        Akron::generate_actions(&s, &mut legal);
        if !legal.contains(&action) {
            return Err(HostError::bad_request("illegal move"));
        }
        Ok(state_to_value(&Akron::apply(s, &action)))
    }
    ...
    fn ai_move(
        ...
        let mut ai = mcts_tune::presets::build_strategy::<Akron>(
            presets(),
            preset,
            custom_spec.as_ref(),
            PRESET_SEED,
        ...
        let (action, search) = mcts_tune::choose_action_with_report(&mut *ai, &s, |action| {
            serde_json::to_value(action).expect("Akron action always serializes")
        });
        let next = Akron::apply(s, &action);
        Ok(AiMoveResult {
    ...
    fn analyze(
        ...
        Ok(mcts_tune::legacy_analysis_with_report(
            ...
            &selected_action,
            search,
            |action| serde_json::to_value(action).expect("Akron action always serializes"),
        ))
    }
    ...
    fn tune_eval(
        ...
        let outcome =
            if let Some(cfg) = baseline_config {
                ...
                // `true`: Akron has a real `Game::zobrist_hash` (already
                ...
                mcts_tune::build_search::<Akron>(&cfg, baseline_seed, true, &budget)?;
                mcts_tune::strategy_tune_eval(
                    ...
                    state_to_value,
                    |_, action| {
                        Some(serde_json::to_value(action).expect("Akron action always serializes"))
                    },
                    trace_path.as_deref(),
                ...
                let baseline_id = baseline
                    ...
                    .expect("games/akron/presets.json must declare at least one preset");
                mcts_tune::strategy_tune_eval(
                    &params,
                    rounds,
                    seed,
                    ...
                    budget,
                    move || {
                        presets().build::<Akron>(&baseline_id, PRESET_SEED).unwrap_or_else(|e| {
                        panic!("games/akron/presets.json's {baseline_id:?} preset must build: {e}")
                    })
                    },
                    initial_state,
                    state_to_value,
...
mod tests {
    ...
    fn legal_moves_and_apply_round_trip() {
        let state = AkronAdapter
            .new_state(serde_json::json!({ "n": MIN_N }))
            .unwrap();
        let moves = AkronAdapter.legal_moves(&state).unwrap();
        assert!(!moves.is_empty());
        let next = AkronAdapter.apply(&state, &moves[0]).unwrap();
        assert_eq!(next["n"].as_u64(), Some(MIN_N as u64));
    }

    /// `Akron::winner` names no one until a span is completed or a player is
    /// forfeited -- `view`'s `winner` field must stay `null` right after the
    /// opening placement, well before either can happen.
    #[test]
    fn view_winner_is_null_before_the_game_is_actually_over() {
        ...
        let moves = AkronAdapter.legal_moves(&state).unwrap();
        let next = AkronAdapter.apply(&state, &moves[0]).unwrap();
        let view = AkronAdapter.view(&next).unwrap();
    ...
    /// the wire format silently drops pile counts across requests.
    #[test]
    fn view_and_state_stay_consistent_across_json_round_trips() {
        let mut state = AkronAdapter
            .new_state(serde_json::json!({ "n": MIN_N }))
            .unwrap();
        for ply in 0..10 {
            let moves = AkronAdapter.legal_moves(&state).unwrap();
            ...
            state = AkronAdapter.apply(&state, &moves[0]).unwrap();
            let view = AkronAdapter.view(&state).unwrap();
