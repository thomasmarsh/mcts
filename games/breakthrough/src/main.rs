...
fn build_easy() -> Box<dyn Search<G = Breakthrough<8, 8>>> {
    Box::new(TreeSearch::<Breakthrough<8, 8>, strategy::Ucb1>::new().config(
        SearchConfig::new().name("breakthrough/easy").expand_threshold(1).max_iterations(100).q_init(QInit::Infinity),
    ))
}

fn build_strong() -> Box<dyn Search<G = Breakthrough<8, 8>>> {
    Box::new(TreeSearch::<Breakthrough<8, 8>, strategy::Ucb1>::new().config(
        SearchConfig::new().name("breakthrough/strong").expand_threshold(0).max_iterations(5000).use_mcts_solver(true).q_init(QInit::Loss),
    ))
}
...
struct PresetEntry {
    id: &'static str, label: &'static str, description: &'static str,
...
impl GameAdapter for BtAdapter {
    fn kind(&self) -> &'static str { "breakthrough" }
    fn label(&self) -> &'static str { "Breakthrough" }
    fn description(&self) -> &'static str { "A fast 8×8 board game where pieces move forward like pawns. First to reach the opponent's back rank wins." }
    fn default_config(&self) -> Value { serde_json::json!({}) }

    fn new_state(&self, _config: Value) -> Result<Value, HostError> {
        Ok(state_to_value(&State::new(BitBoard::new(0x000000000000ffff), BitBoard::new(0xffff000000000000), Player::Black, false)))
    }
    ...
    fn legal_moves(&self, state: &Value) -> Result<Vec<Value>, HostError> {
        ...
        Ok(moves.into_iter().map(|m| Value::Array(vec![Value::from(m.0 as u64), Value::from(m.1 as u64)])).collect())
    }
    ...
    fn apply(&self, state: &Value, mv: &Value) -> Result<Value, HostError> {
        ...
        let action = Move(arr[0].as_u64().ok_or_else(|| HostError::bad_request("invalid from"))? as u8,
                         arr[1].as_u64().ok_or_else(|| HostError::bad_request("invalid to"))? as u8);
        if Breakthrough::<8, 8>::is_terminal(&s) {
    ...
    fn ai_presets(&self) -> Vec<AiPresetInfo> {
        PRESETS.iter().map(|p| AiPresetInfo { id: p.id.into(), label: p.label.into(), description: p.description.into() }).collect()
    }

    fn ai_move(&self, state: &Value, preset: &str) -> Result<AiMoveResult, HostError> {
        let s = value_to_state(state)?;
        let spec = PRESETS.iter().find(|p| p.id == preset).ok_or_else(|| HostError::not_found(format!("unknown preset: {preset}")))?;
        if Breakthrough::<8, 8>::is_terminal(&s) {
            return Err(HostError::bad_request("game is over"));
        }
    ...
    fn analyze(&self, state: &Value, preset: &str, _budget_ms: Option<u64>) -> Result<Analysis, HostError> {
        let s = value_to_state(state)?;
        let spec = PRESETS.iter().find(|p| p.id == preset).ok_or_else(|| HostError::not_found(format!("unknown preset: {preset}")))?;
        if Breakthrough::<8, 8>::is_terminal(&s) {
            return Err(HostError::bad_request("game is over"));
        ...
        Ok(Analysis {
            actions: report.actions.into_iter().map(|a| AnalysisAction {
                action: Value::Array(vec![Value::from(a.action.0 as u64), Value::from(a.action.1 as u64)]),
                visits: a.visits, mean_value: a.mean_value, is_proven: a.is_proven,
            }).collect(),
            principal_variation: report.principal_variation.into_iter().map(|a| Value::Array(vec![Value::from(a.0 as u64), Value::from(a.1 as u64)])).collect(),
            total_visits: report.total_visits, suggested_move: suggested,
        })
    }
}

fn main() { run_stdin_stdout(BtAdapter); }