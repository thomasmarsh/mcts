...
/// `games/breakthrough/presets.json`'s embedded defaults, or an
/// operator-supplied override file named by `BREAKTHROUGH_PRESETS_PATH`
/// (see `PresetTable::load`'s doc comment).
fn presets() -> &'static PresetTable {
    static PRESETS: OnceLock<PresetTable> = OnceLock::new();
    PRESETS.get_or_init(|| {
        let override_path = env::var("BREAKTHROUGH_PRESETS_PATH")
            .ok()
            .map(PathBuf::from);
        PresetTable::load(include_str!("../presets.json"), override_path.as_deref())
            .expect("games/breakthrough/presets.json must parse")
    })
}
...
impl GameAdapter for BtAdapter {
    ...
    fn tune_eval(
        ...
        let outcome = if let Some(cfg) = baseline_config {
            ...
            mcts_tune::build_search::<Breakthrough<8, 8>>(&cfg, baseline_seed, false, &budget)?;
            mcts_tune::strategy_tune_eval(
                ...
                budget,
                move || {
                    mcts_tune::build_search::<Breakthrough<8, 8>>(
                        &cfg,
                        baseline_seed,
                        false,
                        &budget,
            ...
            mcts_tune::strategy_tune_eval(
                ...
                move || {
                    presets()
                        .build::<Breakthrough<8, 8>>("strong", PRESET_SEED)
                        .expect("games/breakthrough/presets.json's \"strong\" preset must build")
                },
                Default::default(),
                trace_path.as_deref(),
                on_game,
