...
fn value_to_state(v: &Value) -> Result<State<8>, HostError> {
    ...
    Ok(State::from_parts(black, white, ko_black, ko_white, turn, w.can_swap, w.winner))
}

fn build_easy() -> Box<dyn Search<G = Gonnect<8>>> {
    Box::new(TreeSearch::<Gonnect<8>, strategy::Ucb1>::new().config(
        SearchConfig::new().name("gonnect/easy").expand_threshold(1).max_iterations(100).q_init(QInit::Infinity)))
}
fn build_strong() -> Box<dyn Search<G = Gonnect<8>>> {
    Box::new(TreeSearch::<Gonnect<8>, strategy::Ucb1>::new().config(
        SearchConfig::new().name("gonnect/strong").expand_threshold(0).max_iterations(5000).use_mcts_solver(true).q_init(QInit::Loss)))
}
...
struct PresetEntry { id: &'static str, build: fn() -> Box<dyn Search<G = Gonnect<8>>> }

struct GonnectAdapter;

impl GameAdapter for GonnectAdapter {
    fn kind(&self) -> &'static str { "gonnect" }
    fn label(&self) -> &'static str { "Gonnect" }
    fn description(&self) -> &'static str { "A Go-like connection game where connecting opposite edges wins." }
    ...
    fn legal_moves(&self, state: &Value) -> Result<Vec<Value>, HostError> {
        ...
        let mut mv = Vec::new();
        if !Gonnect::<8>::is_terminal(&s) { Gonnect::<8>::generate_actions(&s, &mut mv); }
    ...
    fn apply(&self, state: &Value, mv: &Value) -> Result<Value, HostError> {
        ...
        if Gonnect::<8>::is_terminal(&s) { return Err(HostError::bad_request("game is over")); }
...
fn main() { run_stdin_stdout(GonnectAdapter); }