...
use game_tak::{Move, Player, State, Tak, FLAT, MAX_SIZE};
use mcts::game::Game;
use mcts::strategies::mcts::{node::QInit, strategy, SearchConfig, TreeSearch};
use mcts::strategies::Search;

fn player_name(p: Player) -> &'static str { match p { Player::White => "White", Player::Black => "Black" } }
fn parse_player(name: &str) -> Player { match name { "White" => Player::White, "Black" => Player::Black, _ => panic!("invalid player") } }
...
fn state_to_value(s: &State<5>) -> Value {
    let n = 5;
    let cells: Vec<String> = s.cells[..n * n].iter().map(|w| format!("{:016x}", w)).collect();
...
fn build_easy() -> Box<dyn Search<G = Tak<5>>> {
    Box::new(TreeSearch::<Tak<5>, strategy::Ucb1>::new().config(
        SearchConfig::new().name("tak/easy").expand_threshold(1).max_iterations(100).q_init(QInit::Infinity)))
}
fn build_strong() -> Box<dyn Search<G = Tak<5>>> {
    Box::new(TreeSearch::<Tak<5>, strategy::Ucb1>::new().config(
        SearchConfig::new().name("tak/strong").expand_threshold(0).max_iterations(5000).use_mcts_solver(true).q_init(QInit::Loss)))
}
...
struct PresetEntry { id: &'static str, build: fn() -> Box<dyn Search<G = Tak<5>>> }

struct TakAdapter;

impl GameAdapter for TakAdapter {
    fn kind(&self) -> &'static str { "tak" }
    fn label(&self) -> &'static str { "Tak" }
    fn description(&self) -> &'static str { "An abstract strategy game played on a 5x5 board. First to build a road connecting opposite edges wins." }
    ...
    fn legal_moves(&self, state: &Value) -> Result<Vec<Value>, HostError> {
        ...
        let mut mv = Vec::new();
        if !Tak::<5>::is_terminal(&s) { Tak::<5>::generate_actions(&s, &mut mv); }
    ...
    fn apply(&self, state: &Value, mv: &Value) -> Result<Value, HostError> {
        ...
        if Tak::<5>::is_terminal(&s) { return Err(HostError::bad_request("game is over")); }
...
fn main() { run_stdin_stdout(TakAdapter); }