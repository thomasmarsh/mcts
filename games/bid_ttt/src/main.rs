...
struct WireState {
    ...
    chips_x: u16, bid_x: u16,
    chips_o: u16, bid_o: u16,
    tiebreaker: String,
    phase: String,
}

fn piece_name(p: Piece) -> &'static str {
    match p { Piece::X => "X", Piece::O => "O" }
}
fn parse_piece(s: &str) -> Piece {
    match s { "X" => Piece::X, "O" => Piece::O, _ => panic!("invalid piece") }
}

fn state_to_value(s: &BiddingTicTacToe) -> Value {
    serde_json::to_value(WireState {
        board: s.board.iter().map(|p| p.map(|p| piece_name(p).to_string())).collect(),
        chips_x: s.x.chips, bid_x: s.x.bid,
        chips_o: s.o.chips, bid_o: s.o.bid,
...
fn value_to_state(v: &Value) -> Result<BiddingTicTacToe, HostError> {
    ...
    b.x.chips = w.chips_x; b.x.bid = w.bid_x;
    b.o.chips = w.chips_o; b.o.bid = w.bid_o;
    ...
    b.phase = match w.phase.as_str() {
        ...
        _ => return Err(HostError::bad_request(format!("unknown phase: {}", w.phase))),
    };
    Ok(b)
}

fn build_easy() -> Box<dyn Search<G = BiddingTicTacToe>> {
    Box::new(TreeSearch::<BiddingTicTacToe, strategy::Ucb1>::new().config(
        SearchConfig::new().name("bidttt/easy").expand_threshold(1).max_iterations(100).q_init(QInit::Infinity)))
}
fn build_strong() -> Box<dyn Search<G = BiddingTicTacToe>> {
    Box::new(TreeSearch::<BiddingTicTacToe, strategy::Ucb1>::new().config(
        SearchConfig::new().name("bidttt/strong").expand_threshold(0).max_iterations(5000).use_mcts_solver(true).q_init(QInit::Loss)))
}
...
struct PresetEntry { id: &'static str, build: fn() -> Box<dyn Search<G = BiddingTicTacToe>> }

struct BttAdapter;

impl GameAdapter for BttAdapter {
    fn kind(&self) -> &'static str { "bid-ttt" }
    fn label(&self) -> &'static str { "Bidding TicTacToe" }
    fn description(&self) -> &'static str { "Tic-Tac-Toe with bidding for the right to move." }
    ...
    fn legal_moves(&self, state: &Value) -> Result<Vec<Value>, HostError> {
        ...
        let mut mv = Vec::new();
        if !BiddingTicTacToe::is_terminal(&s) { BiddingTicTacToe::generate_actions(&s, &mut mv); }
    ...
    fn apply(&self, state: &Value, mv: &Value) -> Result<Value, HostError> {
        ...
        if BiddingTicTacToe::is_terminal(&s) { return Err(HostError::bad_request("game is over")); }
...
fn main() { run_stdin_stdout(BttAdapter); }