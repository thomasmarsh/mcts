...
//! `QInit`-driven constant `select::SelectPolicy::unvisited_value` uses
...
//! an `Option<Box<dyn PriorPolicyDyn<G>>>` -- a boxed trait object, not a
...
/// terminal), so every `PriorPolicy::evaluate_children` impl here must
...
pub trait PriorPolicy<G: Game>: Clone + Send + Sync {
    ...
    /// `select::SelectPolicy::requirements`'s doc comment for the general
...
impl<G: Game> PriorPolicy<G> for NoPrior {
...
impl<G, E> PriorPolicy<G> for EvaluatorPrior<G, E>
where
    G: Game,
...
impl<G, E> PriorPolicy<G> for NegamaxPrior<G, E>
where
    G: Game,
    E: Evaluator<G> + Default,
{
...
/// Object-safe counterpart of [`PriorPolicy`], letting `SearchConfig` store
/// one behind `Box<dyn PriorPolicyDyn<G>>` without becoming generic over a
/// third strategy type parameter -- see this module's doc comment for why.
/// Blanket-implemented for every `PriorPolicy`; not meant to be implemented
/// directly.
pub trait PriorPolicyDyn<G: Game>: Send + Sync {
    fn evaluate_children(&mut self, state: &G::S, actions: &[G::A]) -> Vec<f64>;
    fn pseudo_visits(&self) -> u32;
    fn requirements(&self) -> config::Requirements;
    fn clone_box(&self) -> Box<dyn PriorPolicyDyn<G>>;
}

impl<G, T> PriorPolicyDyn<G> for T
where
    G: Game,
    T: PriorPolicy<G> + 'static,
{
    fn evaluate_children(&mut self, state: &G::S, actions: &[G::A]) -> Vec<f64> {
        PriorPolicy::evaluate_children(self, state, actions)
    }

    fn pseudo_visits(&self) -> u32 {
        PriorPolicy::pseudo_visits(self)
    }

    fn requirements(&self) -> config::Requirements {
        PriorPolicy::requirements(self)
    }
...
impl<G: Game> Clone for Box<dyn PriorPolicyDyn<G>> {
    fn clone(&self) -> Self {
        self.clone_box()
    }
}
