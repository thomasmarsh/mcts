...
pub struct Trial<G: Game> {
    ...
    /// A `SimulatePolicy`-supplied leaf value for a playout that ended via
...
pub trait SimulatePolicy<G>: Clone + Sync + Send + Default
where
    G: Game,
{
    ...
    /// See `select::SelectPolicy::requirements`'s doc comment -- same
    /// default-from-`backprop_flags` reasoning, mirrored here since
    /// `SimulatePolicy` is a separate trait.
...
impl<G, E> SimulatePolicy<G> for MinimaxRollout<G, E>
where
    G: Game,
    E: Evaluator<G> + Default,
{
...
/// inner `SimulatePolicy` unchanged except that a playout ending via the
...
impl<G, E, S> SimulatePolicy<G> for EvaluatedCutoff<G, E, S>
where
    G: Game,
    E: Evaluator<G> + Default + Clone,
    S: SimulatePolicy<G> + Default,
...
impl<G, S> SimulatePolicy<G> for EpsilonGreedy<G, S>
where
    G: Game,
    S: SimulatePolicy<G>,
{
    ...
    fn select_move<'a>(
        ...
        if rng.gen::<f64>() < self.epsilon {
            <Uniform as SimulatePolicy<G>>::select_move(
                &mut Uniform,
                state,
                available,
                stats,
...
impl<G, S> SimulatePolicy<G> for DecisiveMove<G, S>
where
    G: Game,
    S: SimulatePolicy<G> + Default,
{
...
impl<G> SimulatePolicy<G> for Mast
where
    G: Game,
{
    fn label(&self) -> String {
...
impl<G> SimulatePolicy<G> for Nst
where
    G: Game,
{
    fn label(&self) -> String {
...
impl<G, S> SimulatePolicy<G> for Lgr<G, S>
where
    G: Game,
    S: SimulatePolicy<G> + Default,
{
...
/// from this table (`backprop::BackpropPolicy::update`'s `flags.lgr2()`
...
impl<G, S> SimulatePolicy<G> for Lgr2<G, S>
where
    G: Game,
    S: SimulatePolicy<G> + Default,
{
