...
use crate::algorithms::mcts::correction::residual_correction;
use crate::algorithms::mcts::index;
use crate::algorithms::mcts::index::Id;
use crate::algorithms::mcts::node;
use crate::algorithms::mcts::node::real_action;
...
pub struct Shared<'a, G: Game> {
    ...
    /// `BackpropPolicy::update`'s `canonicalizes` param) -- distinct from
...
pub fn expand<'a, G: Game>(
    ...
    node.expand(|| {
        ...
        if matches!(status, TerminalStatus::NotTerminal) {
            ...
            // `prior::PriorPolicy`'s expansion-time seeding (MCTS-IP/MS) --
...
/// otherwise call `SelectPolicy::best_child` (`select_step` below,
...
/// the same way across all 10 `SelectPolicy` impls, so this lives here
...
fn reborrow_prior<'p, G: Game>(
    prior: &'p mut Option<&mut (dyn crate::algorithms::mcts::prior::PriorPolicyDyn<G> + 'static)>,
) -> Option<&'p mut (dyn crate::algorithms::mcts::prior::PriorPolicyDyn<G> + 'static)> {
    match prior {
        Some(p) => Some(&mut **p),
        None => None,
    }
        ...
        let best_idx =
            match proven_win_child::<G>(shared.use_mcts_solver, node, shared.index, player) {
                ...
                None => {
                    ...
                    match &ismcts_legal {
                        Some(legal_idxs) => crate::algorithms::mcts::select::ismcts_best_child(
                            &select_ctx,
                            node.children(),
                            legal_idxs,
                            select_strategy,
...
pub fn simulate_step<G: Game>(
    ...
    global: &TreeStats<G>,
    simulate_strategy: &mut impl SimulatePolicy<G>,
    state: &G::S,
...
pub fn backprop_step<G: Game>(
    shared: &Shared<'_, G>,
    stack: &[(Id, usize)],
    backprop_strategy: &impl BackpropPolicy,
    trial: Trial<G>,
    flags: BackpropFlags,
) {
...
/// `BackpropPolicy::update`'s own `graph_stats`-gated stats loop -- and
