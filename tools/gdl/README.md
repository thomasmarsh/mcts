...
deleted.** It used to independently re-prove Tic-Tac-Toe/Hex by
lowering `lud/Tic-Tac-Toe.lud`/`lud/Hex.lud` through Ludii's own ludeme AST, but
...
**Backend codegen has a first real implementation:** `src/codegen/rect.rs`
...
Three tests cross-check the three independent implementations of
Tic-Tac-Toe now in this repo: `tests/oracle.rs` (`style_c` + `core::interp` vs. hand-written
`games/ttt`), `tests/ttt_gen_vs_interp.rs` (`games/ttt-gen` vs. `core::interp`), and
`tests/ttt_gen_oracle.rs` (`games/ttt-gen` vs. hand-written `games/ttt`
directly, via the `mcts::game::Game` trait on both sides — the "point of
