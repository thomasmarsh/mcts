...
enum Player {
    Ucb { search: TreeSearch<Standard, UcbProfile>, net: NTupleValueNet, cfg: GumbelConfig },
    Completed { search: TreeSearch<Standard, CompletedProfile>, net: NTupleValueNet, cfg: GumbelConfig },
}

impl Player {
    fn ucb(net: NTupleValueNet, policy: NTuplePolicyNet, cfg: GumbelConfig, seed: u64) -> Self {
        Self::Ucb {
            search: TreeSearch::default().config(SearchConfig::default()
                .expand_threshold(1).max_playout_depth(0).q_init(QInit::Loss)
                .simulate(EvaluatedCutoff::new().evaluator(net.clone()))
                .with_policy_logits(policy).seed(seed)),
            net, cfg,
        }
    }

    fn completed(net: NTupleValueNet, policy: NTuplePolicyNet, cfg: GumbelConfig, seed: u64) -> Self {
        Self::Completed {
            search: TreeSearch::default().config(SearchConfig::default()
                .expand_threshold(1).max_playout_depth(0).q_init(QInit::Loss)
    ...
    fn choose(&mut self, state: &State<6, 7>) -> (Move, usize) {
        match self {
            Self::Ucb { search, net, cfg } => {
                let action = gumbel_search_with_root_value(search, state, cfg, net.value(state) as f64).action;
...
fn compare(name: &str, games: usize, make_candidate: impl Fn(u64) -> Player, make_base: impl Fn(u64) -> Player) {
    ...
    for game in 0..games {
        ...
        let (winner, game_nodes) = if candidate_first { play(&mut candidate, &mut base) } else { play(&mut base, &mut candidate) };
        nodes += game_nodes;
        match winner {
            None => draws += 1,
    ...
    println!("{name}: {wins}-{draws}-{losses} W-D-L, score share {share:.3}, wall {:.2}s, nodes {nodes}", start.elapsed().as_secs_f64());
}

fn load_value(path: &str) -> NTupleValueNet { NTupleValueNet::load(path).expect("value weights") }
fn load_policy(path: &str) -> NTuplePolicyNet { NTuplePolicyNet::load(path).expect("policy weights") }

fn main() {
    let args: Vec<_> = std::env::args().collect();
    if args.len() < 5 { panic!("usage: gumbel_connect4_fidelity <zero.bin> <zero.policy.bin> <trained.bin> <trained.policy.bin> [games] [sims]"); }
    let games = args.get(5).map_or(100, |s| s.parse().expect("games"));
    let sims = args.get(6).map_or(32, |s| s.parse().expect("sims"));
    let zero = load_value(&args[1]); let zero_policy = load_policy(&args[2]);
    let trained = load_value(&args[3]); let trained_policy = load_policy(&args[4]);
    let old_root = GumbelConfig { sims, use_completed_q: false, ..GumbelConfig::default() };
    let full_root = GumbelConfig { sims, ..GumbelConfig::default() };
    compare("1 uniform policy + old root Q + UCB1 interior", games,
        |seed| Player::ucb(zero.clone(), zero_policy.clone(), old_root, seed),
        |seed| Player::ucb(zero.clone(), zero_policy.clone(), old_root, seed ^ 0xBAD5EED));
    compare("2 learned policy + old root Q + UCB1 interior", games,
        |seed| Player::ucb(trained.clone(), trained_policy.clone(), old_root, seed),
        |seed| Player::ucb(zero.clone(), zero_policy.clone(), old_root, seed ^ 0xBAD5EED));
    compare("3 learned policy + completed-Q root + UCB1 interior", games,
        |seed| Player::ucb(trained.clone(), trained_policy.clone(), full_root, seed),
        |seed| Player::ucb(zero.clone(), zero_policy.clone(), old_root, seed ^ 0xBAD5EED));
    compare("4 learned policy + completed-Q root + completed-Q interior", games,
