...
/// or absolute). Matches the ignored `local/runs/` directory.
pub const BENCH_RUNS_DIR: &str = "local/runs/bench";
...
/// stdout is redirected to `local/runs/bench/<run_id>/log.jsonl` and its stderr to
/// `local/runs/bench/<run_id>/stdout.log`. The child is placed in its own process
...
/// same identity as the run registry and database.
