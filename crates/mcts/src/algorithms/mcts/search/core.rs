...
use crate::algorithms::mcts::search::shared::Shared;
...
use crate::algorithms::mcts::search::SearchContext;
    ...
    G::S: std::fmt::Display,
{
    ...
    pub fn select_final_action(&mut self, state: &G::S) -> G::A {
        ...
        let stack = crate::algorithms::mcts::stack::NodeStack::new(vec![(self.root_id, 0)]);
        ...
        stats.table_bytes = legacy_entries
            * (std::mem::size_of::<u64>() + std::mem::size_of::<Id>())
            + stats.graph_table_entries
                * (std::mem::size_of::<crate::algorithms::mcts::table::TranspositionKey>()
                    + std::mem::size_of::<Id>());
        stats
    }
    ...
    pub(crate) fn compute_pv(&mut self, init_state: &G::S, selected_root_action: Option<&G::A>) {
        ...
        while node.is_expanded() && !(bound_descent && stack.len() > self.config.max_playout_depth)
        {
            ...
            node_id = crate::algorithms::mcts::search::shared::verified_child_id::<G>(
                &crate::algorithms::mcts::search::shared::TranspositionCtx {
                    index: &self.index,
                    table: &self.table,
                    explicit_dag: matches!(self.config.graph_search, GraphSearch::Dag(_)),
                    keying: self.config.transposition_keying,
