...
pub struct PlayerStats {
    ...
    // `BackpropPolicy::provides_posterior` (`config.rs`) reject any
    // config where a select policy that reads these is paired with a
    // backprop policy that does not write them.
...
impl NodeStats {
    ...
    /// backprop policy has written it.
    ...
    /// based select policy reading `sum_squared_score` (UCB1-Tuned, RAVE)
...
/// player -- what `SelectPolicy::score_child` typically needs (visits,
...
impl<A: Action> ChildArray<A> {
    ...
    /// One lock acquisition covering every field a `SelectPolicy` typically
    ...
    /// `Node`/`Id` exists for that slot -- `prior::PriorPolicy`'s
    ...
    /// `-value` (see `prior::PriorPolicy`'s doc comment on this same
    /// restriction).
...
struct SolverState {
    ...
    // read) when `use_mcts_solver` is off or no GPN select policy is
