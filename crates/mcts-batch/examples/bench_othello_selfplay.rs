//! Real self-play throughput: batched-CPU self-play (this crate's
//! `gumbel_explore`, one call per ply covering every still-live game in
//! the batch) vs. the existing per-node engine
//! (`crates/mcts`'s `TreeSearch`/`GumbelCompletedQ`/`EvaluatedCutoff`, one
//! game played start-to-finish before the next begins -- the same shape
//! `games/othello/examples/mlx_selfplay_bench.rs` already benchmarks the
//! CPU/MLX evaluator split on), same evaluator (`CnnValueNet`, CPU, all-
//! zero weights -- correctness of the output doesn't matter for a
//! throughput measurement, only latency), same `sims`/`num_considered`.
//!
...
fn play_games_per_node(games: usize, sims: u32) -> (usize, f64) {
    type Profile = Mcts<GumbelCompletedQ, EvaluatedCutoff<Othello, CnnValueNet>>;
...
fn main() {
    ...
    println!("running {games} games at {sims} sims/move (all-zero weights, CPU evaluator both sides)...");
