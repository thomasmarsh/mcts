...
use crate::util::{AnySearch, Verbosity};

use super::log::LogRecord;
use super::games::BenchGame;
...
fn write_match_result<W: Write>(
    ...
    line.push('\n');
    // Ignore write errors -- the run process can still keep playing
    // matches even if the log stream is broken.
...
mod tests {
    use super::*;
    use crate::bench::games::{BenchGame, StrategyInfo};
    ...
    impl BenchGame for RockPaperScissors {
        ...
        fn play_match(&self, strategy_a: &str, strategy_b: &str) -> crate::bench::games::MatchOutcome {
            let winner = match (strategy_a, strategy_b) {
                ...
                _ => None, // same strategy or no rule = draw
            };
            crate::bench::games::MatchOutcome {
                winner,
                extra: None,
            }
        }
    ...
    fn test_round_robin_bench_emits_correct_number_of_match_records() {
        ...
        // Ordered pairs: each unordered pair is played twice (i,j) and (j,i),
        // so each strategy plays 4 games: 2 as first arg, 2 as second arg.
        // a beats b (both orderings), loses to c (both orderings) -> 2 wins, 2 losses.
    ...
    fn test_round_robin_bench_self_match_is_draw() {
        // If we only pass one strategy, there are no pairs (i != j), so
        // no games are played and all results are zero.
        let game = RockPaperScissors;
    ...
    fn test_round_robin_bench_multiple_rounds_aggregates_correctly() {
        ...
        // Each strategy: 4 games per round * 3 rounds = 12 games
        assert_eq!(results[0].total(), 12);
        assert_eq!(results[0].wins, 6); // 2 wins per round * 3
