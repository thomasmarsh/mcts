...
for g in $(seq 0 $((GENS - 1))); do
  ...
  "$BIN" dump --label gumbel \
    ...
    --games "$GAMES" --seed "$g" --sims "$SIMS" \
    --weights "$RUN_DIR/weights/gen_$g.bin"

  # Replay window: the last up to three generations of shards.
  lo=$((g - 2)); [ "$lo" -lt 0 ] && lo=0
  parts=""
  for s in $(seq "$lo" "$g"); do parts="$parts$ROOT/$RUN_DIR/shards/gen_$s.bin,"; done
