...
//! other crate in this workspace is source-controlled. Parses a `style-c/sexpr/*.sc` file through
//! `ludii::style_c::parse_game`, lowers the resulting `Program` through `ludii::codegen::generate`,
...
//!
//! ```text
//! cargo run -p ludii --bin codegen -- style-c/sexpr/tic-tac-toe.sc TicTacToe "Tic-Tac-Toe" \
...
/// Pipes `source` through `rustfmt` -- the template in `ludii::codegen::rect` builds its method
...
fn main() -> ExitCode {
    ...
    let [_, sexpr_path, struct_name, game_name] = args.as_slice() else {
        eprintln!(
            "usage: codegen <path/to/game.sc> <StructName> <\"Game Name\">\n\n\
    ...
    let program = match ludii::style_c::parse_game(&source) {
        Ok(p) => p,
        Err(e) => {
    ...
    match ludii::codegen::generate(game_name, struct_name, sexpr_path, &program) {
        Ok(rust_source) => {
